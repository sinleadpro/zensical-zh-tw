...
| `pricing` | 計費規則 | 選 | 僅說明扣款機制（如 Cyber 幣預扣），不含具體費率或單價 |
...
3. **計費或規則說明**(若有) { #pricing-{category} } — 僅說明扣款機制與操作前提，**不含費率、單價、計費標準表格**
...
3. **選擇出貨方式：** 勾選商品後，於下方選單選擇出貨方式(例如「黑貓託運單」)，最後點擊 **「確認出貨」**。
...
>
> [^1]: 一般版立即扣抵 Cyber 幣；PLUS版/企業版列入對帳單。
> [^2]: 順豐 OpenAPI
> ```
>
...
12. ☐ 文件不含收費/運費/內部資訊/舊版方案名稱（§E 第 10-14 項）
...
10. **不寫入收費資訊** — 不含費率、單價、計費標準表格、費用 FAQ（如「每封 NT$1」「1 Cyber 幣」「70 字計費」）
11. **不寫入運費資訊** — 不含運費數字、運費計算公式、含運費欄位的截圖（如託運單確認視窗、單號使用紀錄截圖）
12. **不含內部專用內容** — 不使用 `{% if is_internal %}` 區塊、不暴露方案差異對照表、加值功能對照、API 內部細節
13. **不含私人通訊** — 不出現客服對話、Slack 訊息、內部通訊截圖
14. **不使用舊版方案名稱** — 達人版、尊爵版、旗艦版、白金版等已停用命名

**允許保留**：操作前提（如「需先儲值 Cyber 幣」）、字數限制提醒（如「上限 70 字」）、UI 欄位名稱（如「費用」欄）、「客製化」作為操作動詞
...
[^1]: 一般版若餘額不足會直接擋下。
[^2]: 一般版立即扣抵 Cyber 幣；PLUS版/企業版列入對帳單。
```
...
文件 A:
  ...
  頁面: {{https://demo005.cyberbiz.co/admin/products/69132755/descriptions}} 
  主題: {{使用文字編輯器編輯內容}}
  檔案路徑: docs/{{pos}}/{{topic-kebab-case}}.md
```
