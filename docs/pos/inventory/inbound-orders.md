...
進倉單不限於手動建立，系統會根據不同通路的庫存需求，自動連動產生對應單據：

=== "我方主動建立(我方入倉)"

    由我方人員於後台手動建立進倉單，指定由 **第三方** 倉進貨至門市。
    ...
    → [了解進倉完整流程](inbound-complete-process.md)
...
=== "接收進倉請求(他方出倉)"
    ...
    → [了解出倉完整流程](outbound-complete-process.md)
...
=== "完成調撥協作（調倉轉單）"
    ...
    **第二步**
    ...
    → [了解調倉完整流程](transfer-complete-process.md)
...
=== "於後台操作"
    ...
    2. 點擊 **進倉單**，點擊 **新增**。
    3. 設定 **預計到貨日**。
    4. 新增出倉品項，輸入 SKU 或以名稱搜尋商品，並填寫數量。
    5. 點擊 **新增進倉**，完成出貨申請。
    
    !!! tip "批量操作"
        若品項眾多，可點擊 **匯入**，下載範例 Excel 填寫後上傳。不支援同時發貨至多個門市。
...
=== "於前台操作"
    ...
    1. 在 POS 前台點選 **庫存 > 進倉單**。
    2. 點擊 **新增進倉單**，設定 **預計到倉日期**。
    3. 掃描商品條碼並輸入數量。
    4. 點擊 **新增進倉**，待確實收到貨物後，即可開始點收。
...
| 順序 | 狀態 | 說明 | 我方下一步操作 |
| ---- | --- | ---- | ------------- | 
| 1 | 待確認 | 單據已建立，正等待收貨方核准 | [確認 / 取消進倉](inbound-orders/#確認--取消進倉) | 
| 2 | 待入庫 | 收貨方已核准，等待發起端發貨 | [確認 / 取消收貨](inbound-orders/#確認--取消收貨) |
| 3 | 處理中 | 確認收貨後，單據就會變為此狀態 | [收貨清點](inbound-orders/#收貨清點) |
| 4 | 部分點收 | 貨品分次抵達，盤點數未達總數<br>待貨品全部抵達，可再次執行盤點 | [收貨清點](inbound-orders/#收貨清點) | 
...
=== "於後台操作"
    ...
    3. 點選 **開始盤點**，輸入實際到貨量後點擊 **送出盤點結果**。
        ...
        > **狀態同步**： <br> 我方 **進倉單** 自動切換為 **已入庫** 狀態 / 對方 **出倉單** 自動切換為 **已出庫** 狀態
...
=== "於前台操作"    
    ...
    4. 使用掃描槍清點數量，完成後點擊 **送出盤點結果**。
        ...
        > **狀態同步**：<br> 我方的 **進倉單** 自動切換為 **已入庫** 狀態 / 對方的 **出倉單** 自動切換為 **已出庫** 狀態
...
- **了解歷程**：點擊 **詳情**，即可查看單據變更狀態的 **時間紀錄**。
    ...
    ![](../../assets/images/POS-後台-全通路庫存管理-進倉單-單號歷程01.png){ .screenshot }
...
- :lucide-arrow-right:{ .lg }   
  [__進倉完整流程__](inbound-complete-process.md)       
