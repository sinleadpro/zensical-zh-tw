...
![POS 營收分析頁面](../../assets/images/pos-bi-pos-revenue-hero.png){ title="POS 營收分析頁面" .hero-page }
...
- **僅計入認列訂單**：報表所有數字僅計入認列訂單，已取消、需退貨的訂單不列入。詳見[認列訂單定義](#specs-pos-revenue-counted-orders){ title="認列訂單定義" }。
...
| 比較器 | 適合分析 | 呈現方式 |
| :-- | :-- | :-- |
| [時間區間比較器](#operate-pos-revenue-window) | 同一批門市在不同年份或不同月份的營業趨勢 | POS 業績分店趨勢圖（折線圖） |
| [週數時段比較器](#operate-pos-revenue-week) | 一段期間內各星期、各時段的營業分布 | 星期週數業績表＋時段銷售累計圖 |
| [各分店比較器](#operate-pos-revenue-store) | 所有門市在指定區間的營業排名 | 業績比較排名表 |
...
1. **選取 POS 店：** 在「請選取或搜尋您的POS店」欄位選擇要納入的門市，可同時選取多家；不選取則預設涵蓋全部門市。
    ...
    ![選取 POS 店畫面](../../assets/images/pos-bi-revenue-window-select-store.png){ title="選取 POS 店畫面" }

2. **選擇比較類型：** 在「請選取時間區間」選擇 **「不同年份數據比較（YOY）」** 或 **「不同月份數據比較（MOM）」**。

    ![選擇比較類型畫面](../../assets/images/pos-bi-revenue-window-select-type.png){ title="選擇比較類型畫面" }

3. **新增比較區間：** 點擊 **「＋ 新增年份」**（或新增月份）加入要對比的年份或月份，最多可選擇 10 組[^1]。

    ![新增比較區間畫面](../../assets/images/pos-bi-revenue-window-add-interval.png){ title="新增比較區間畫面" }
...
5. **切換指標：** 點選圖表上方的 **「營業額」**、**「訂單數」**、**「平均客單價」**、**「消費人數」**、**「人均消費額」** 按鈕，即可切換查看不同指標的趨勢折線圖。
    ...
    === "營業額"
        ...
        ![營業額趨勢圖](../../assets/images/pos-bi-revenue-window-metric-revenue.png){ title="營業額趨勢圖" }

    === "訂單數"

        ![訂單數趨勢圖](../../assets/images/pos-bi-revenue-window-metric-orders.png){ title="訂單數趨勢圖" }

    === "平均客單價"

        ![平均客單價趨勢圖](../../assets/images/pos-bi-revenue-window-metric-aov.png){ title="平均客單價趨勢圖" }

    === "消費人數"

        ![消費人數趨勢圖](../../assets/images/pos-bi-revenue-window-metric-customer.png){ title="消費人數趨勢圖" }

    === "人均消費額"

        ![人均消費額趨勢圖](../../assets/images/pos-bi-revenue-window-metric-per-customer.png){ title="人均消費額趨勢圖" }
...
1. **選擇時間區間：** 在「請選取時間區間」選擇要分析的起訖日期，區間不得超過三個月[^2]。
    ...
    ![選擇時間區間畫面](../../assets/images/pos-bi-revenue-week-select-range.png){ title="選擇時間區間畫面" }

2. **選取 POS 店：** 在「請選取或搜尋您的POS店」欄位選擇要納入的門市，可同時選取多家；不選取則預設涵蓋全部門市。

    ![選取 POS 店畫面](../../assets/images/pos-bi-revenue-week-select-store.png){ title="選取 POS 店畫面" }

3. **送出查詢：** 點擊 **「搜尋」**。
4. **查看星期週數業績表：** 上方的「星期週數業績表」會列出各星期（週一至週日）的營業指標，可比較週間的客流與營收分布。

    ![星期週數業績表畫面](../../assets/images/pos-bi-revenue-week-weekday-table.png){ title="星期週數業績表畫面" }

5. **查看時段銷售累計：** 下方的「時段銷售累計表」以折線圖呈現各時段（00:00～23:59）的營業表現。

    ![時段銷售累計圖畫面](../../assets/images/pos-bi-revenue-week-hourly-chart.png){ title="時段銷售累計圖畫面" }

6. **切換指標：** 點選 **「營業額」**、**「訂單數」**、**「平均客單價」**、**「消費人數」**、**「人均消費額」** 按鈕，切換表格與圖表呈現的指標。

    ![切換指標畫面](../../assets/images/pos-bi-revenue-week-switch-metric.png){ title="切換指標畫面" }
...
1. **選擇日期區間：** 在頁面上方的日期欄位選擇要查詢的起訖日期。
    ...
    ![選擇日期區間畫面](../../assets/images/pos-bi-revenue-store-select-range.png){ title="選擇日期區間畫面" }
...
3. **查看排名表：** 系統會以「業績比較表」列出所有門市，預設依銷售額由高到低排名，並一併呈現訂單數、消費人數、日均消費額、平均客單價與人均消費額。各欄位意義見[各分店比較器欄位對照表](references/pos-revenue-metrics-reference.md#reference-pos-revenue-store-columns){ data-preview }。
    ...
    ![各分店排名表畫面](../../assets/images/pos-bi-revenue-store-ranking-table.png){ title="各分店排名表畫面" }
...
- :lucide-dollar-sign:{ .lg }  
  [__營收分析__](../../ec/business-intelligence/revenue-analysis.md){ title="營收分析" }  
  深入看全店營收組成、毛利與成長趨勢。
