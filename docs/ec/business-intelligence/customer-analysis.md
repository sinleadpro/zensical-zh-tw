...
## 應用參考 { #appendix-customer-analysis }

消費顧客分析除了可以幫助您觀察行銷活動成效，也能協助制定優惠活動的消費門檻策略。以下公式可作為制定門檻的參考依據：

??? example "新客行銷"

    **新客首購平均訂單金額：**（新客訂單金額 – 新客回購訂單金額）÷（新客訂單數 – 新客回購訂單數）

    用於設定新客專屬優惠券、期間限定首購禮、新客滿額贈等活動的消費門檻。

??? example "舊客行銷"

    **舊客平均訂單金額：** 舊客訂單金額 ÷ 舊客訂單數

    用於設定舊客專屬優惠券、舊客滿額贈等活動的消費門檻。

??? example "全館行銷"

    **平均訂單金額：** 總訂單金額 ÷ 總訂單數

    用於設定全館優惠券、全館滿額贈等活動的消費門檻。
