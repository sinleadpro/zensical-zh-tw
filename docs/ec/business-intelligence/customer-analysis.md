...
1. **看顧客數：** 「顧客概況」以數據卡呈現 **累積顧客數**[^4]、 **消費顧客數**[^5]、 **消費新客數**[^2] 與 **消費舊客數**[^3]，將滑鼠移到卡片的提示圖示上，會顯示各數字的定義。
...
2. **看新舊客貢獻：** 切換後，數據卡會分別顯示 **總訂單數**[^6]**(額)**[^9]、 **新客訂單數**[^7]**(額)**[^10] 與 **舊客訂單數**[^8]**(額)**[^11]，搭配下方的區間分析與佔比圖，看出訂單與營收主要由哪一群顧客帶來。
...
[^2]: 第一次下單發生在指定時間內的顧客數。
[^3]: 過去已有下訂單紀錄，且在指定時間內又再次下單的顧客。
[^4]: 開店截至指定時間結束前，曾經下單的顧客數。備註：下單指的是下訂有效訂單，即訂單狀態為非取消、非退貨訂單。
[^5]: 指定時間內，有下單的顧客數。
[^6]: 指定時間內的有效訂單數。有效訂單：訂單狀態為非取消、非退貨訂單。
[^7]: 指定時間內，新客所帶來的有效訂單數。
[^8]: 指定時間內，舊客所帶來的有效訂單數。
[^9]: 指定時間內的有效訂單金額。
[^10]: 指定時間內，新客所帶來的有效訂單金額。
[^11]: 指定時間內，舊客所帶來的有效訂單金額。
