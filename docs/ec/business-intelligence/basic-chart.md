...
## 使用前提與限制 { #prerequisites-basic-chart }

!!! plan "方案 / 開通條件"
    「基本報表」是多數方案內建的功能，出現在後台「分析報表」選單下。若您的選單中找不到這個項目，通常代表目前方案改為提供功能更完整的相關分析報表；如需確認，請聯絡客服。
...
??? quote "左側選單找不到「基本報表」？"
    ...
    這通常和方案有關。「基本報表」是多數方案的內建功能；若您使用的是更高階方案，系統會改提供功能更完整的「營運圖表分析」。如需確認目前方案的報表權限，請聯絡客服。
