...
| :-- | :-- | :-- |
| 總會員數 | 數字 | 開店至今累計的會員總數 |
| 近30日新註冊會員 | 數字 | 最近 30 日新加入的會員數 |
| 會員數趨勢 | 折線圖 | 每月新會員數隨時間的變化 |
| 期間內註冊會員消費次數(會員總數佔比) | 圖表 | 區間內註冊會員中購買 1 次、2 次、3 次以上、無購買紀錄各佔多少 |
| 會員平均認列訂單數 月趨勢 | 折線圖 | 總認列訂單數 ÷ 總會員數 隨月份的變化 |
| 會員平均回購週期天數 | 數字 | 會員兩次下單之間的平均間隔天數 |
| 會員人均消費額 月趨勢 | 折線圖 | 每月平均每位購買會員貢獻的金額 |
| 新會員下單率 | 圖表 | 當月新註冊會員中，有下單與未下單的比例(分「所有非取消訂單」與「認列訂單」兩種) |
| 會員回購率 | 數字 | 下過 2 次以上訂單的客戶數 ÷ 曾下過訂單的客戶數 |
...
### 掌握回購與會員貢獻 { #operate-member-analysis-repurchase }

1. **看回購週期：** 「會員平均回購週期天數」估算會員兩次下單之間的平均間隔，可作為回購推播時機的參考(例如在接近平均週期前提醒會員回購)。
2. **看人均消費：** 「會員人均消費額 月趨勢」呈現每月平均每位購買會員貢獻的金額，觀察客單貢獻是上升或下滑。
3. **看平均訂單數：** 「會員平均認列訂單數 月趨勢」為總認列訂單數除以總會員數，反映平均每位會員帶來的訂單量。
4. **看回購率：** 「會員回購率」為下過 2 次以上訂單的客戶數，除以曾下過訂單的客戶數，是衡量黏著度的核心指標。
...
??? quote "進入頁面顯示方案不支援"
    [](){ #faq-member-analysis-plan }
    這表示您目前的方案尚未包含會員分析功能。

    - 基本版、進階版、專業版，以及體驗 / 試用版不支援。
    - 請聯絡客服或您的開店顧問，確認方案是否可升級。
