...
### 方案開通條件 { #prerequisites-periodic-orders-plan }

定期定額分析需要您的方案同時開通「定期定額」功能與數據分析功能才能使用。

- [x] **定期定額功能：** 需開通定期定額（訂閱制）功能，才會有定期訂單資料可供分析。
- [x] **數據分析功能：** 需方案支援後台「分析報表」的圖表分析功能。

!!! plan "方案 / 開通條件"
    定期定額分析屬於 **企業版** 方案的功能。若您的方案未包含此功能，進入頁面時會看到「您目前方案不支援，請聯絡客服或開店顧問詢問」的提示。此時請洽客服或您的開店顧問，確認方案是否可升級或加購。
...
- **本頁無匯出功能：** 定期定額分析僅供線上檢視，頁面沒有下載 / 匯出按鈕。若需要逐筆資料，請至定期定額訂單列表查看。
...
??? quote "進入頁面顯示「您目前方案不支援」"
    [](){ #faq-periodic-orders-plan }
    這表示您目前的方案尚未包含定期定額分析功能。

    - 定期定額分析屬於企業版方案功能。
    - 請聯絡客服或您的開店顧問，確認方案是否可升級或加購。
