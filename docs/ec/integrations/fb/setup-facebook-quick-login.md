...
- [x] **網域重新導向：** 請確認後台「網域管理」中，「[總是將顧客重新導向到這裡](../../website-management/domain-management.md#cyberbiz-後台新增網域){ title="網域管理" }」勾選的是 Facebook 所設定的網域。
- [x] **信箱限制：** 若消費者使用 Hinet 信箱，可能會因 Hinet 阻擋訊息而無法收到忘記密碼等系統通知，建議引導使用其他信箱。
- [x] **帳號合併機制：** 若顧客的 Facebook 信箱與 Google 或 LINE 登入使用的信箱相同，系統會自動將其判定為同一會員並進行 **帳號合併**。
...
### 使用自訂圖示 <small>需建立 Meta 應用程式</small>
...
2. 填寫以下欄位：
    - **應用程式網域**：填入官網 **自有網域**。
        - **格式要求**：填寫網址時，請勿加入 `https://` 前綴。
          -  若您的網址為 `https://example.com`，請填入 `example.com`
        - **子網域相容**：若您的網址帶有 `www` 子網域，請務必將根網域與 `www` 子網域分別填入。
          -  根網域： `example.com`
          -  子網域： `www.example.com`
