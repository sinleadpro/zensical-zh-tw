...
此功能使商家能夠將 CYBERBIZ 平台上的產品數據（包含資訊、庫存、價格，以及最重要的 **商品影片**）自動同步到 Meta 的產品目錄中。設定完成後，後續在建立 Meta 廣告時若選擇「[**目錄型廣告**](設定 Meta 廣告活動.md#廣告呈現效果){ title="設定 Meta 廣告活動" }」，廣告投放時商品影片與商品圖即會輪播展示。

## 設定前提與功能限制

在進行同步前，請務必確認滿足以下條件與規格：

- [x] **商品影片上傳：** 商家必須已於 CYBERBIZ 後台完成[商品影片上傳](../../../products/create-and-manage/設定商品影片.md){ title="設定商品影片" }。
- [x] **擴充套件串接：** 商家必須具備並已串接「[Facebook 新版商業擴充套件](../mbe/設定 FBE 帳號授權與資產連結.md){ title="設定 FBE 帳號授權與資產連結" }」功能。
...
2.  **進行資產連結：** 此步驟與 [FBE 帳號授權與資產連結](../mbe/設定 FBE 帳號授權與資產連結.md#操作步驟教學){ title="設定 FBE 帳號授權與資產連結" } 的「連結企業資產」流程相同（包含登入 Facebook 帳號、同意授權、選擇資產）。
...
- :lucide-rocket:{ .lg }   
  [__投放 Meta 目錄型廣告__](設定 Meta 廣告活動.md){ title="設定 Meta 廣告活動" }       
