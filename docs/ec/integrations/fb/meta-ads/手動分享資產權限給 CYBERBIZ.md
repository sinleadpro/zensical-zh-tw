...
請進入您的「[企業管理平台設定 :lucide-external-link:](https://business.facebook.com/latest/settings)」，依序對 [**粉絲專頁**](#分享粉絲專頁權限){ data-preview }、[**像素**](#分享像素權限){ data-preview } 及 [**目錄**](#分享目錄權限){ data-preview } 進行權限指派。
...
1. 登入 [企業管理平台設定](https://business.facebook.com/latest/settings)，於左側選單點選「資料來源」>「**像素**」或「**資料集**」。
