...
3.  **前往網域廠商設定**：
    ...
    === "遠傳（原亞太）"
        ...
        !!! warning "原「亞太 e 管家」DNS 後台已關閉"
            亞太電信與遠傳電信合併後，原「亞太 e 管家」DNS 管理後台已於 **2025 年正式關閉**。請改登入 **遠傳 Seednet 網址註冊中心 (RS)** 進行網域驗證。

        請依下列步驟於 Seednet 平台完成 FB 網域驗證：

        1. **登入平台**：前往 [Seednet RS 網址註冊中心](https://seed.net.tw)。
        2. **進入設定**：點選「我的域名管理」 > 點選網域 > 「DNS 代管設定」。
        3. **新增紀錄**：
            *   **類型**：選擇 `TXT`。
            *   **主機名稱**：填入 `@`。
            *   **紀錄值**：貼上 FB 提供的 `facebook-domain-verification=...` 字串。
