...
-   :lucide-tags:{ .lg .middle } __建立並驗證 GMC 帳號__
    ...
    [:lucide-arrow-right: 設定教學](setup-google-merchant-center.md){ title="設定 Google Merchant Center 並同步 CYBERBIZ 商品" }    
...
-   :lucide-chart-no-axes-column-increasing:{ .lg .middle } __建立並串接 GA4 帳號__
    ...
    [:lucide-arrow-right: 設定教學](ga/setup-google-analytics.md){ title="建立並串接 Google Analytics" }
...
2.  **連結 GMC 與 GA4**：在 Merchant Center 設定中啟用自動標記，並將 [GA4 連結至 Merchant Center](設定%20GMC%20重要事件來源追蹤與自動標記.md#將-ga4-連結至-merchant-center){ data-preview }。
