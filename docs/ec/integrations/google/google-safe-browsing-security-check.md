...
2.  **進入 [Google Search Console :lucide-external-link:](https://search.google.com/search-console) (GSC)**：
    ...
    *   *提醒：若您尚未將網站加入 GSC，必須先完成「[網站所有權驗證](setup-google-search-console.md){ title="註冊並驗證 Google Search Console" }」才能進行後續操作。*
...
- :lucide-monitor-smartphone:{ .lg }  
  [__二階段驗證__](../../website-management/setup-manage-two-factor-auth.md){ title="設定與管理二階段驗證" }       
