...
> :lucide-triangle-alert:  LINE Messaging API channel 與 Provider 綁定後便無法修改，請務必確認連動到正確的 Provider。
    ...
    !!! info "什麼是服務提供者 (Provider)？" 
	    Provider 代表提供服務的品牌主體。若要確保「LINE 登入」與「Messaging API」的資料互通（例如同步會員 UID），兩者必須設定在 **同一個 Provider** 下。詳情請參閱 [LINE 官方文件說明 :lucide-external-link:](https://tw.linebiz.com/manual/line-official-account/line-porvider-and-channel-intro/)。
...
操作說明請參閱 [__LINE OA 訊息樣板設定__](設定超商配送限制與物流排除.md)。
...
- :lucide-layout-template:{ .lg }     
  [__LINE OA 訊息樣板設定__](綁定 LINE 帳號與官網會員)  
  。
