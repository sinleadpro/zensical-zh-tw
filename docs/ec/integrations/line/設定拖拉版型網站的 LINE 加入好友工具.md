...
在進行官網設定前，請先至 **LINE Official Account Manager** 取得宣傳素材：

1. 登入 [**LINE Official Account Manager** :lucide-external-link:](https://manager.line.biz)，進入 **主頁 > 增加好友人數 > 增加好友工具**。
2. 在此處您可以選擇 [複製網址](../../notifications/發送 LINE 加入好友邀請.md#get-line-oa-add-friend-link){ data-preview }  、下載 **「行動條碼 (QR Code)」** 或複製 **「加入好友按鈕」** 的語法。
...
- **設定步驟**：
    1. 新增一個圖片區塊。
    2. **上傳圖片**：建議在電腦/平板/手機版圖片中上傳「加入好友行動條碼」圖片。
    3. **縮圖設定**：商家可自行製作縮圖，並根據前台畫面調整縮圖大小。
    4. **圖片連結**：貼上從 LINE OAM 複製的「好友網址」。
    5. **儲存設定**。
...
- **版型限制**：本教學僅適用於「拖拉版型」的網站設定。
- **預先設置**：在開始操作前，請確保已自行設置好品牌官網的 LINE OA 帳號。
- **儲存習慣**：每項設定完成後，請務必記得點擊「儲存設定」以確保更動生效。
