...
- :lucide-phone:{ .lg }   
  [__同步會員手機號碼__](setup-line-quick-login-phone-number.md){ title="設定 LINE 快速登入時取得會員手機號碼" }       
  申請 LINE 認證權限後，系統將以手機號碼作為優先識別碼，確保會員資料庫的精確度與唯一性。

- :lucide-link:{ .lg }     
  [__配置 LIFF 一鍵登入__](setup-liff-auto-login-member-binding.md){ title="設定 LIFF 自動登入與會員綁定" }    
