...
登入 CYBERBIZ 管理後台，前往 **商品 > 商品自訂分類** 即可進入管理介面。

介面功能說明：

- **欄位顯示**：可勾選顯示於商品列表的欄位。
- **搜尋商品**：輸入商品名稱或 SKU 快速搜尋目標商品。
...
2. 在群組列表下方輸入新群組名稱，點擊 **新增群組**，建立新的商品自訂分類群組。
...
3. 點擊 **儲存** 套用變更。
4. **設定群組頁橫幅**：新增圖片、影片或文字，作為行銷或重點資訊展示。
...
- :lucide-filter:{ .lg }   
  [__條件分類群組__](設定商品條件分類群組)  
  設定條件讓系統自動將商品分類。
- :lucide-eye-off:{ .lg }  
  [__秘密商品群組__](設定秘密商品群組)   
  建立秘密商店，僅提供購買連結特定顧客。
- :lucide-percent:{ .lg }    
  [__設定商品加價購__](../marketing/設定商品加價購.md)  
