...
系統透過圖片的「相對位置」自動配對款式實照與色票圖示。請將圖片上傳至「商品圖片」區塊（上傳規範請參考 [商品圖片設定](create-update-products.md#商品圖片){ title="新增與更新商品" }），並遵循 「先圖後色」 的分段排序邏輯。
...
??? quote "此功能是否適用於所有版型？"
    ...
    本教學僅適用於 **預設版型**。若您使用的](product-swatches-variant-images-drag-drop.md)能教學](設定商品色票與款式圖片-拖拉版型.md){ title="設定商品色票與款式圖片-拖拉版型" }。
