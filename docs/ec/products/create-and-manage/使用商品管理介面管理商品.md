...
last_modified: 2026-06-14 13:11
lang: zh-TW
type: quickstart
status: ""
version: ""
...
??? warning "排除搜尋的影響"
    ...
    - **站內搜尋**：無法透過商店搜尋框找到商品。瞭解 [如何設定站內搜尋](../discovery/setup-site-search.md){ data-preview }
...
- [__新增與更新商品__](create-update-products.md){ title="新增與更新商品" }   
建立單一商品並上架
...
=== ":lucide-toggle-left: 單筆排除"
    ...
    2. 在商品列表中找到 **商品搜尋** 欄位。若未顯示，請拖曳下方滾動條。
    3. 啟用 / 關閉 **商品搜尋** 選項。
    > *開啟 (ON)*（商品可被搜尋）；*關閉 (OFF)*（商品無法被搜尋）。[了解更多](product-search-visibility.md#商品排除搜尋效果)。
...
使用 Excel 範本一次更新大量商品。
- [__批次操作多款式商品__](create-update-products.md#operate-product-create-variants){ title="新增與更新商品" }  
同步更新多款式商品資訊。

</div>

## 庫存列表
> :lucide-navigation: 檢視已[開啟管理庫存功能](create-update-products.md#operate-product-create-inventory){ title="新增與更新商品" }以及庫存設定有限數量的商品。

![](../../../assets/images/ec-inventory-list.zh-tw.png)

<div class="grid cards" markdown>

- [__庫存水位通知__](create-update-products.md#operate-product-create-inventory){ title="新增與更新商品" }  
設定庫存低於一定數量會主動通知商家。
...
##](../discoverability/product-search-visibility.md)ty/設定商品搜尋可見性.md)
- [批次修改商品](../bulk-operations/batch-update-product-descriptions-shipping.md)
