...
開始前，以下功能會影響您看到的欄位與可用選項。多數為內建，部分為加值功能或與經營站別相關，一般商店不一定全部具備：

<!-- - [x] **管理庫存(內建)：** 所有方案皆可在款式管理中開啟，無需額外開通。 -->
- [x] **會員專屬價格(VIP)：** 需先建立 VIP 群組，才能在款式管理中設定各群組專屬售價。
- [x] **商品成本價、款式材積：** 為加值功能，開通後才會在款式中出現「成本價」「材積(CM)」欄位。
- [x] **商品影片：** 為加值功能，且僅支援 **拖拉版型** 的主題，前台才能正常顯示。
- [x] **自訂欄位：** 為加值功能，開通後會在編輯商品時出現「自訂欄位」區塊。
- [x] **運送溫層與物流綁定：** 為加值功能，開通後會在 **設定** 分頁出現溫層與物流配送設定。
...
!!! plan "方案 / 開通條件"
    若不確定您的店家是否已開通成本價、材積、商品影片、自訂欄位、溫層綁定或前台多國功能，請聯繫您的 CYBERBIZ 業務窗口確認。
...
3. **填寫商品標語與簡述：** 顯示在商品名稱附近的特色或促銷短句，以及商品的簡短描述（[客製說明](編輯商品簡述與商品標語.md){ title="編輯商品簡述與商品標語" }）。
...
!!! plan "方案 / 加值功能"
    [**商品影片**](setup-product-videos.md){ title="設定商品影片" } 為加值功能，且僅支援 **拖拉版型** 的主題，前台才會顯示。
...
1. **公開 / 不公開：** 切換商品在前台是否陳列販售。設為 **不公開** 後，前台隱藏且連結無法購買。
2. **複製：** 以現有商品為範本快速建立新商品；系統會要求先填寫新款式的 **商品編號(SKU)** 再複製[^copy-sku]。
...
若已開通 **前台多國功能**，商品編輯頁標題右側會出現 **語言切換** 選單，可為每一種語言分別填寫商品的文字內容。一般單一語言的商店可略過本節。
...
CYBERBIZ 沒有獨立的「跨境商品」類型，跨境銷售所需的設定 **分散在一般欄位中**。經營海外站別或使用海外倉儲時，以下設定與跨境直接相關：
...
- **商品課稅類別(設定分頁)：** 經營 **日本站** 的台灣商家，可於設定分頁選擇商品課稅類別(10%、8%、0%)，預設為 10%；如需調整預設稅率，請聯繫您的客戶經理[^japan-tax]。
...
[^japan-tax]: 此課稅類別選項僅在「台灣商家經營日本站」的情境下顯示。
...
??? quote "找不到「材積(CM)」或「成本價」欄位？"
    [](){ #faq-product-no-meas }
    材積與成本價皆為加值功能，開通後才會在款式中顯示。若僅看到「重量(KG)」「售價」而無這些欄位，代表尚未開通，請聯繫業務窗口。
...
<!-- | **商品標語/簡述** | 輸入吸引消費者的行銷短語。<br> 支援純文字，或[切換 HTML 模式進行進階排版](編輯商品簡述與商品標語.md){ title="編輯商品簡述與商品標語" }。 | — | -->
...
<!-- * **商品標語/商品簡述**：輸入吸引消費者的行銷短語。支援純文字輸入，或切換至 HTML 模式進行進階排版。[進階設定說明](編輯商品簡述與商品標語.md){ title="編輯商品簡述與商品標語" } -->
