 ...
 *商品簡述* 與 *商品標語* 能快速傳達商品特色與賣點，提升瀏覽體驗並增加轉換率。商家可以透過以下兩種方式客製文字樣式，維持品牌一致性並強化行銷效果：

- [個別文案修改](#個別商品文案修改)：修改個別商品的文案樣式
- [全站樣式調整](#全站樣式調整)：修改全站 CSS 設定檔案

> :lucide-triangle-alert: 商品標語與商品簡述欄位的語法設定 *優先* 於樣板編輯器的全站樣式。若同時修改樣板編輯器和個別商品，系統將以個別商品設定為準。
...
3. 在 **商品資訊** 分頁中的 **商品標語** 和 **商品描述** 欄位，輸入資訊，可[直接以 HTML 語法編輯](#產生 HTML 樣式碼)。  
4. 點擊 **儲存** 套用變更。

![](../../assets/images/ec-product-slogan-product-brief-fields.zh-tw.png)

### :material-numeric-1-circle-outline: 產生 HTML 樣式碼
...
### :material-numeric-2-circle-outline: 驗證前台顯示
前往商品頁面，確認文字樣式已依設定修改。
...
### :material-numeric-1-circle-outline: 查找目標 CSS 樣式
...
### :material-numeric-2-circle-outline: 修改 CSS 樣式
...
### :material-numeric-3-circle-outline: 驗證前台顯示
