---
title: 商品管理快速上手
description: 快速了解 CYBERBIZ 商品管理後台功能，搜尋、篩選、批次操作與商品可見性設定。
product:
  - EC
module:
  - 商品
activ: activate
surfaces:
  - backend
devices: []
apis: []
type: guide
features: []
tasks:
  - 商品管理介面總覽
  - 後台搜尋商品
  - 商品排除搜尋
  - 批次修改商品
tnb: trunk
plans:
  - PLUS
  - 高手
  - 企業
lang: zh-TW
sites:
  - TW
status: review
doc_next: ""
doc_previous: ""
tags: []
difficulty: beginner
notes:
  - Overview 與操作流程互聯
  - Quickstart 上手指南
---
# 商品管理快速上手
掌握商品管理後台的基本操作，包括搜尋、篩選、批次操作。
{ .subtitle }

## 所有商品頁面總覽
所有商品頁面的主要功能區塊與操作按鈕。
> 後台路徑：登入 CYBERBIZ 管理後台，前往 **商品 > 所有商品**。

### 新增商品

![](../../assets/images/ec-all-products-add.zh-tw.png) 

### 商品列表
使用以下功能快速搜尋、篩選與管理商品列表。

- `請輸入商品名、SKU`：使用關鍵字快速搜尋商品
- `進階搜尋`：使用條件篩選商品
- `已選取_個商品`：選擇商品進行批量作業
- `顯示_個商品`：調整每頁顯示商品筆數

![](../../assets/images/ec-all-products-products-list.zh-tw.png)

### 快捷按鈕
- :lucide-eye: 選擇是否公開商品
- :lucide-external-link: 快速前往商品前台
- :lucide-trash-2: 刪除商品

![](../../assets/images/ec-all-products-shortcuts.zh-tw.png)

## 後台搜尋商品

### 關鍵字搜尋
輸入 *商品名稱* 或 SKU 快速搜尋商品。

1. 登入 CYBERBIZ 管理後台，前往 **商品 > 所有商品**。
2. 在頁面上方的搜尋欄位中，輸入欲查詢的 **商品名稱** 或 **SKU**。
3. 按下 ++enter++ 鍵或點擊「搜尋圖示」:material-magnify:，系統將顯示符合條件的商品列表。

![](../../assets/images/ec-products-search-bar.zh-tw.png)

### 進階條件篩選
使用篩選器套用多項特定條件，以精確定位目標商品。

1. 登入 CYBERBIZ 管理後台，前往 **商品 > 所有商品**。
2. 點擊搜尋欄位旁的 **進階搜尋**，展開下拉選單。
3. 點選一個或多個 **篩選群組欄位**，如 **商品類型**、**商品廠商**、**商品標籤**。
> 實際可使用的篩選群組欄位因方案或後台設定而異。
4. 選擇各欄位的篩選條件。
5. 點擊 **搜尋**，系統將顯示符合所有條件的商品列表。

![](../../assets/images/ec-products-filters.zh-tw.png)

#### 篩選欄位的運算邏輯
系統會依篩選欄位，自動套用對應的邏輯規則：

| 篩選群組欄位    | 套用邏輯               | 範例條件                 | 篩選結果說明                 |
| --------- | ------------------ | -------------------- | ---------------------- |
| **同欄位條件** | 聯集（OR 邏輯）：滿足任一條件即符合 | 類型：`衣服` 跟 `鞋子`       | 顯示所有屬於「衣服」或「鞋子」類型的商品   |
| **跨欄位條件** | 交集（AND 邏輯）：必須同時滿足所有條件 | 類型：`衣服` 且 上架狀態：`已上架` | 顯示同時符合「衣服」類型且「已上架」的商品。 |
	
### 組合搜尋條件
結合 **關鍵字搜尋** 與 **進階條件篩選**，設定多重篩選邏輯，鎖定符合複雜條件的特定商品。

1. 設定[進階篩選條件](#進階條件篩選)： 點擊搜尋欄位旁的 **進階搜尋**，選擇商品屬性或分類，點擊 **搜尋** 以套用篩選條件。
2. [輸入關鍵字]([#關鍵字搜尋)： 在搜尋欄位中輸入商品名稱或 SKU，按下 ++enter++ 鍵或點擊「搜尋圖示」（:material-magnify:）以進一步限縮搜尋結果。

![](../../assets/images/ec-products-search-filter-combo.zh-tw.png)

!!! tip "下一步：篩選商品後進行批次操作"
	篩選出特定商品清單後，可進一步進行[大量選取動作](#選取大量商品)，再進行[批次操作](#批次操作)一次性對大量商品進行操作。

## 商品排除搜尋
設定商品是否可被站內搜尋或 Google 索引。

1. 登入 CYBERBIZ 管理後台，前往 **商品 > 所有商品**。
2. 在商品列表中找到 **商品搜尋** 欄位。
> 若未顯示，請拖曳下方滾動條。
3. 啟用 / 關閉 **商品搜尋** :lucide-toggle-right: 選項
> `開啟 (ON)`（商品可被搜尋）。    
> `關閉 (OFF)`（商品無法被搜尋）。

![](../../assets/images/ec-product-list-search-index-toggle.png)

!!! note "關閉商品搜尋效果"

	- CYBERBIZ 站內無法搜尋到該商品。
	- 前台 collection all 商品頁不顯示商品。
	- Google 搜尋引擎不索引。
	
	> 更多設定途徑，請參考[設定商品排除搜尋](設定商品排除搜尋)。


## 選取大量商品

### 當頁所有商品

1. 登入 CYBERBIZ 管理後台，前往 **商品 > 所有商品**。
2. 點擊 **Select** 操作選單中的 :lucide-square: ，可一次選取當頁顯示的所有商品 。
> 點擊 **顯示 __ 個商品**，可以調整商品列表每頁顯示的商品數量。

![](../../assets/images/ec-products-select-whole-page.zh-tw.gif)


### 全館商品

1. 清除所有搜尋跟篩選條件。
2. 點擊 **已選取__個商品**，選擇 **選取全部商品**。

![](../../assets/images/ec-products-select-all-products.zh-tw.gif)

### 符合條件的所有商品

1. [進行商品檢索](#後台搜尋商品)，產生篩選後的商品列表清單。
2. 點擊 **已選取__個商品**，選擇 **選取全部商品**。

![](../../assets/images/ec-products-select-all-results.zh-tw.gif)


## 批次操作

1. [選取多個商品後](#選取大量商品)。
2. 點擊 **Select** 操作選單 ，選擇批次操作選項。
> :lucide-lock: 可使用的批次操作選項因方案而異。

![](../../assets/images/ec-products-batch-process.zh-tw.gif)

??? note "批次操作選單項目"
	- 商品公開 / 隱藏
	- 刪除商品
	- 匯出商品
	- 開啟搜尋 / 排除搜尋
	- 編輯會員專屬價格 :lucide-lock:
	- 匯出會員專屬價格 :lucide-lock:
	- 開啟搜尋
	- 排除搜尋
	- 複製商品至門市 :lucide-lock: | POS
	
	> :lucide-lock: 僅適用特定方案與產品。選項內容可能隨時間變動。
	
!!! warning "批次操作所有商品限制"
	為維持系統效能與資料安全，選取全部商品時，部分操作功能將受到限制。建議使用篩選功能或分批選取商品，每批操作完成後確認結果，確保資料正確。


## 常見問題
??? quote "為什麼搜尋不到商品？"
	- 確認關鍵字或 SKU 正確
	- 檢查篩選條件是否過多
	- 商品是否已刪除或不可見

??? quote "搜尋結果可以匯出嗎？"
	目前不支援直接匯出，有商品匯出需求請使用 [Excel 匯出功能](批次修改商品資訊#### 匯出商品 Excel 表格)

??? quote "可以儲存篩選條件嗎？"
	目前系統不支援儲存。

## 延伸閱讀

- [商品排除搜尋](商品排除搜尋)
- [批次修改商品](批次修改商品資訊)
