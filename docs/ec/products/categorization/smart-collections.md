...
last_modified: 2026-06-04 11:25
lang: zh-TW
type: tutorial
status:
version:
...
- :lucide-package:{ .lg }
  [__檢查商品資料__](../products/index.md){ data-preview }
  確認商品的名稱、類型、廠商、標籤與價格填寫完整，條件分類才能正確命中。

- :lucide-layout-template:{ .lg }
  [__設定前台選單與版型__](../website-appearance/index.md){ data-preview }
  把建立好的條件分類放進前台選單或頁面，讓顧客逛得到。

- :lucide-languages:{ .lg }
  [__查看支援語言__](references/multi-language-supported.md){ data-preview }
  確認多國語系可翻譯的語言清單。
...
??? quote "「商品條件分類」和「自訂群組」差在哪？"
    ...
    **商品條件分類** 是設定條件、由系統自動篩選商品；**自訂群組** 則是逐一手動挑選商品加入。前者適合會持續變動的分類(如「1,000
元以下」)，後者適合需要精準控制成員的分類(如活動精選)。
