...
- [x] **排除特定關鍵字（進階）：** 可透過修改程式碼 `search.liquid`，設定含有[特定字眼的商品不出現在搜尋結果中](../discoverability/設定搜尋結果中排除特定關鍵字商品.md){ data-preview }  。
...
- :lucide-folder-edit:{ .lg }  
  [__商品自訂分類__](custom-collections.md){ data-preview }    
  自訂商品分類群組。
