...
*   **搜尋範圍**：系統預設的搜尋結果包含以下欄位：
    ...
    | 商品群組名稱 | 包含自訂分類跟條件分類等商品群組的名稱。<br> （ 如商品網址 `https://xxx/collections/mygroup` 中的 `mygroup`) | [自訂分類設定](../categorization/custom-collections.md#加入與移出商品){ data-preview }、[條件分類設定](../categorization/設定商品條件分類群組.md#建立商品條件分類){ data-preview }。 |
