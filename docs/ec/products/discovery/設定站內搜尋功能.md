...
- :lucide-toggle-left:{ .lg }   
  [__全站搜尋欄位開關__](../../website-appearance/使用拖拉版型設計網站版面與首頁區塊.md#導覽列){ data-preview }       
  進入後台「網站外觀」>「套版主題管理」>「網站設定」>「全站設定」（或導覽列設定），選擇是否顯示搜尋欄位。

- :lucide-package-x:{ .lg }   
  [__個別商品排除搜尋__](../快速上手商品管理.md#商品排除搜尋){ data-preview }       
  可在商品列表的「商品搜尋」欄位直接開/關，或利用批次操作勾選商品後選擇「排除搜尋」。

- :lucide-code:{ .lg }   
  [__透過關鍵字排除（進階）__](設定搜尋結果中排除特定關鍵字商品.md){ data-preview }       
  若要建立秘密賣場或排除含有特定字眼的商品，可進入「程式碼編輯器」修改 `search.liquid` 檔案。
