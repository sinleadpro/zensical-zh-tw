...
> 請依實際需求評估並選擇圖床服務，CYBERBIZ 不提供第三方圖床之設定、操作或技術支援。
...
商品 Excel 檔案中的 A、B 欄位預設為隱藏。系統會根據這兩個欄位是否帶有系統數值，在上傳 Excel 檔案後判斷為新增商品或更新既有商品。
...
!!! warning "注意事項"
	請勿使用粗體字，否則會導致上傳格式錯誤。部分欄位支援性質不同，例如：限第一次上傳、可重複多次上傳、單一欄位、可逗號複選欄位、支援 HTML 等。
...
|狀態|內容說明|建議行動|
|---|---|---|
|失敗|顯示錯誤訊息或失敗原因|修正後重傳|
|成功|請耐心等候系統完成匯入作業，完成後會收到通知信件。|請勿重複上傳|
|完成|匯入已確認完成|可檢查結果或進行其他內容更新|
