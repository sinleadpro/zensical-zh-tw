---
title: 設定商品色票與款式圖片對應
description: 設定拖拉版型商品的色票功能，讓前台消費者能透過款式選單即時看到對應的圖片。
created: 2026-04-14 14:55
last_modified: 2026-04-14 15:30
lang: zh-TW
type: tutorial
status: 
author: Jase
version: 
reviewers: 
notes: 
ga_views: 0
feedback: 0
products:
  - EC
modules: []
sites:
  - TW
audiences:
  - admin
difficulty: intermediate
tnb: trunk
plans:
  - 專業
  - 進階
  - 高手
  - 專業 PLUS 
  - 進階 PLUS
  - 高手 PLUS
  - 企業
cyb_extensions: []
intents: []
features: 
  - 拖拉版型
prerequisites: []
related: []
tags:
  - 色票
  - 款式圖片
  - 拖拉版型
acoiv: 
apis: []
devices:
  - desktop
  - mobile
ui_components: []
paths: []
layouts: []
wp_url: 
  - https://www.cyberbiz.io/helpcenter/?p=4833
  - https://www.cyberbiz.io/support/?p=31319
permalink: 
comments: false
search:
  exclude: false
icon: lucide/swatch-book
hide: []
---

{ .subtitle }

[:lucide-bolt:{ title=\"適用功能\" }](../../resources/conventions#適用功能) | 拖拉版型
{ .doc-badge }

{ .hero-page }

## 商品色票功能說明

CYBERBIZ 的**拖拉版型色票功能，主要目的是為了讓消費者在前台瀏覽商品時，能透過點選特定款式（如顏色、穿搭、規格等），即時呈現該款式對應的圖片，幫助消費者了解商品細節並提升購買意願。

## 功能適用條件

*   此功能**僅限使用「拖拉版型」**的用戶使用。
*   適用於企業版、PLUS 版等支援拖拉版型的方案。

## 後台設定步驟

**【後台路徑】：** `商品` → `所有商品` → 選擇欲編輯的商品。

1.  **建立多款式商品**：
    *   完成商品基本設定後，點選「建立多款式商品」。
    *   點選「新增規格」，將規格設定為「**顏色**」（或其他欲展示的款式名稱），並新增細項項目（如：紅色、藍色）後按下「儲存」。
2.  **上傳款式對應圖片**：
    *   儲存規格後，規格名稱前方會自動出現**圖片視窗圖示**。
    *   點選該圖示後，可透過「資料夾拖進檔案」或「選擇上傳圖片」兩種方式上傳款式圖。
    *   上傳後，針對該特定款式選擇對應的圖片。
3.  **確認圖片同步**：
    *   在款式選單中上傳的圖片，也會同步顯示在商品主圖清單中。
4.  **儲存並預覽**：
    *   設定完成後，點選上方「儲存設定」，並點擊「前往該商品」查看前台呈現效果。

## 開啟前台顯示（全站設定）

若要在商品列表頁（如首頁、分類頁）顯示色票小圖，必須完成以下全站設定：

**【後台路徑】：** `網站外觀` → `套版主題管理` → `網站設定` → `全站設定`。

1.  進入「全站設定」頁面。
2.  找到「商品相關設定」區塊。
3.  勾選啟動「**顯示商品色票小圖**」功能。
4.  **顯示位置**：完成後，色票小圖會出現在商品群組頁、商品多層級分類頁等列表頁面。

## 前台呈現效果

*   **尚未選擇款式前**：消費者可以查看該商品的所有產品圖。
*   **選擇特定款式後**：系統會自動篩選並僅顯示與該款式（如特定顏色）關聯的圖片。
*   **色票小圖**：在商品列表中，商品下方會出現顏色小點或圖片，方便消費者直接點選預覽不同款式的外觀。

**💡 注意事項：**
*   **紅利商城**目前不支援色票功能。
*   若要讓 Facebook 商店的商品圖片也能隨顏色正確變換，建議務必開啟此功能並依照顏色順序放置圖片。

您是否需要我進一步說明，如何利用「加強型評估」在 GA4 中追蹤消費者點選這些色票的行為成效？

## 後續操作

<div class="grid cards" markdown>

- :lucide-store{ .lg }   
  [__Facebook 商店設定__](../../integrations/fb/mbe/設定 Facebook 跟 Instagram 商店.md){ data-preview }       
  完成色票設定後，可進一步設定 Facebook 與 Instagram 商店，讓商品圖片能在社群平台上呈現多款式效果。

</div>


## 常見問題

