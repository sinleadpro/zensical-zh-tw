...
![超商配送限制與物流排除](../../../assets/images/ec-product-variants-volumn-weight.zh-tw.png){ title="超商配送限制與物流排除" .hero-page }

## 超商配送限制說明 { #intro-cvs-shipping-restrictions }

超商物流（B2C／C2C）對包裹的材積與重量有明確限制，超過規格即無法配送。CYBERBIZ 系統會在結帳時自動計算訂單中所有商品的材積與重量總和，若超過超商限制，結帳頁便不會顯示超商取貨選項。

您可以透過以下方式控制商品是否適用超商物流：

- **設定材積與重量：** 在商品編輯頁的款式管理中填寫，系統依此自動判斷。
- **手動排除超商配送：** 將材積或重量設定為超過限制值，強制該商品不得使用超商物流。
...
??? info "超商物流限制規格"
    ...
    !!! warning "超商物流的尺寸與重量限制可能隨業者政策調整，實際可配送規格請以各超商物流業者最新公告為準。"

??? tip "以規格條件排除配送選項"
    你可以透過調整商品的材積（長寬高總和）或重量，使其超過超商物流限制，系統便會自動將該商品排除於超商配送選項之外。瞭解 [如何設定指定商品無法使用超商物流](#setup-cvs-shipping-exclusion)。
...
3.  在 **款式管理** 中，找到商品的材積與重量欄位：
    ...
    !!! tip "若材積或重量欄位未填寫，系統預設為 `0`，即視為符合所有配送條件。"

4.  點擊 **儲存** 以套用變更。

![商品材積與重量設定](../../../assets/images/ec-product-volume-weight-setting.zh-tw.png){ title="商品材積與重量設定" }
...
## 設定指定商品無法使用超商物流 { #setup-cvs-shipping-exclusion }
...
### 2. 商品總材積符合限制 <small>含未填寫欄位</small>
