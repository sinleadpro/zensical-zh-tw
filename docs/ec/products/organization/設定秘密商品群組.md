...
- [x] **關閉商品站內搜尋：**
    - 進入 **商品 > 所有商品** > 點進該商品頁面的 **商品資訊** 或直接點擊秘密群組中的商品名稱，進入頁面。
    - 將「**商品搜尋功能**」設定為 **關閉**。
...
- [x] **排除特定關鍵字（進階）：** 可透過修改程式碼 `search.liquid`，設定[含有特定字眼的商品不出現在搜尋結果中](在搜尋結果中排除特定關鍵字商品.md){ data-preview }  。

## 秘密群組的應用情境

秘密群組常搭配「推薦人分潤」功能使用，作為網紅或特定團主的導購工具：

- **專屬分潤連結：** 商家可將秘密群組的網址加上[推薦人代碼](../../profit-sharing/設定推薦人分潤方案#步驟-4綁定推薦對象並取得代碼)（例如：`www.yourshop.com/collections/秘密群組?rcode=RAYTEST`）貼給特定對象。
...
- **恢復機制：** 若在修改程式碼（如 `search.liquid`）時發生錯誤，可利用編輯器中的「查看之前版本」功能[回溯至預設版本](../../website-appearance/使用樣板編輯器恢復網頁代碼.md){ data-preview }  。
