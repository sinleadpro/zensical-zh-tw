...
audiences:
  ...
  - 店主
wp_url: 
  - [[archive/wordpress/wp-reg-20260106/商品多層級分類設定|商品多層級分類設定]]
  - [[archive/wordpress/wp-ent-20260106/商品多層級分類設定|商品多層級分類設定]]
notes:
  - verify FAQ
...
[:lucide-lock:{ title="適用方案" }](conventions.md#適用方案) | PLUS / 企業  [:lucide-toggle-right:{ title="適用功能" }](conventions.md#適用功能) |  拖拉版型
{ .doc-badge }
...
3. 在大類別下，點選 **新增**。
	...
	> :lucide-info: 小類別即為商品分類群組。相關設定請參閱 [設定自訂分類群組](設定商品自訂分類群組.md)、[設定商品條件分類群組](設定商品條件分類群組.md)、[商品類型](編輯商品描述與商品設定#進階設定)。
