...
[:lucide-lock:{ title="適用方案" }](../../resources/conventions#適用方案) | 進階(PLUS) / 高手(PLUS) / 企業  [:lucide-toggle-right:{ title="適用功能" }](../../resources/conventions#適用功能) | 拖拉版型
{ .doc-badge }
...
!!! info "自動生成商品規格篩選器"  
	系統會抓取商店內的所有商品規格生成篩選器。可透過[編輯商品規格](#步驟四編輯商品規格)修改顯示項目。
