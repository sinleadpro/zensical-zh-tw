...
| **門檻判定** | 以「折抵前」的訂單金額作為門檻基準 | 1500 元訂單可同時套用門檻為 $1000、$900、$800 的三張券 |
| **標籤限制** | 綁定標籤的優惠券僅針對該標籤商品進行折抵 | 若購物車包含一般商品 A 與標籤商品 B，標籤優惠券僅會計算商品 B 的折抵金額 |

!!! example "多張優惠券併用計算範例"
    **情境假設：**

    - 購物車內容：商品 A （ $1000，一般商品）、商品 B （ $500，綁定 `特定標籤`）
    - 持有優惠券：
        1. **全館券**：折 $50
        2. **個人標籤券**：9 折 （僅限 `特定標籤`）
    ...
    **運算方式：**
    ...
    1. **套用全館券 （$50）**：依商品金額比例分攤，商品 A 折 $34、商品 B 折 $16。
    2. **套用個人標籤券 （9 折）**：僅針對商品 B 剩餘金額折抵，$（500 - 16） ✕ 10% = $48。
    3. **套用全館標籤券 （73 折）**：僅針對商品 B 剩餘金額折抵，$（500 - 16 - 48） ✕ 27% = $117。

    **最終結果：**

    - **商品 A**：總共折抵 $34。
    - **商品 B**：總共折抵 $16 + 48 + 117 = $181。
