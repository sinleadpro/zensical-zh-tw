...
3. 設定核心參數：
    ...
    - **其他通路有效訂單套用消費紅利發送**：當手動替會員新增 [其他通路有效訂單]() 時是否同步發送紅利。**(企業版專用)**
...
可於 [匯出訂單報表]()時，將同步呈現紅利折抵與金額換算之詳細欄位：
...
- :lucide-bell-ring:{ .lg }
  [__設定紅利點數到期通知__](優惠券碼紅利點數到期通知.md){ title="優惠券（碼）與紅利點數到期通知" }
