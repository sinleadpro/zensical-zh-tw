...
!!! tip "應用情境"
    - **套餐組合**：主餐（群組 A）選 1 件 + 飲料（群組 B）選 1 件，特價 199 元。
    - **穿搭組合**：上衣選 1 件 + 下著選 1 件 + 配件選 1 件，現折 300 元。
    - **加購策略**：購買主商品（群組 A）1 件，即可用 99 元加購指定配件（群組 B）1 件。
...
| 規則類型 | 說明 | 範例 |
| :--- | :--- | :--- |
| **固定金額** | 組合後的總價為固定數值 | A(1)+B(1) = 999 元 |
| **固定折扣** | 組合後的商品打指定折扣 | A(1)+B(1) 享 85 折 |
| **折固定金額** | 組合後的總價減去固定金額 | A(1)+B(1) 現折 100 元 |
| **每件折固定金額** | 組合內每件商品各減去固定金額 | A(1)+B(1) 每件再折 10 元 |
