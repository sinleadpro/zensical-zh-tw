...
「任選折扣」可針對一組商品設定「買 N 件享優惠」的階梯式規則，例如「3 件 599 元」、「3 件 7 折」。顧客在結帳時，系統會優先比對並套用此規則，適合用於季末清倉、跨品項組合促銷等情境。

!!! tip "應用情境"
    - **季末清倉**：設定「2 件 500 元、3 件 699 元」，加速中低價位商品去化。
    - **新品帶舊款**：組合新舊商品並設定「2 件 8 折」，提高整體曝光與銷售。
    - **多品項促銷**：將高、中、低價位商品混搭，設定「3 件 999 元」拉高平均客單價。

## 使用須知

- **商品限制**：一個商品只能加入一個任選折扣群組。
- **門檻計算**：可設定多組階梯規則（如 2 件 699 元、3 件 599 元），系統將自動套用最優惠門檻。
...
3. **設定剩餘商品計價**：選擇 **固定折扣** 、 **每件折固定金額** 規則類型，可進一步選擇剩餘商品是否沿用優惠。
    ...
    !!! example "計價範例：3 件 599 元"

        - 若選「以優惠計算」：買 4 件時，前 3 件共 599 元，第 4 件按單件優惠價 199.67 元計算。
        - 若選「以原價計算」：買 4 件時，前 3 件共 599 元，第 4 件按商品原價計算。
