...
last_modified: 2026-08-28 17:40
lang: zh-TW
type: guide
author: ""
reviewers: []
...
- **優惠套用順序**：同一筆訂單中，系統將依 **優惠排序數值** 套用折扣，數值越大者優先計算。
- **分類疊加規則**：同時將 **大分類** 與其底下的 **中分類/小分類** 加入同一活動時，優惠 **僅計算一次**，不會重複套用。
- **折扣計算方式**：折扣計算方式為：**每件商品折扣 × 符合條件商品數量**，而非僅對整筆訂單折扣一次。
- **折扣計算基準**：多層級分類滿額折扣依 **訂單原價** 判斷消費金額門檻。
