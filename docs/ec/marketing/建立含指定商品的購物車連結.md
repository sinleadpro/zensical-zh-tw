...
- **基本格式**：`[您的網址]/cart/replace?products=[款式ID]:[數量],[款式ID]:[數量]`
- **參數說明**

| 參數 | 必填 | 說明 |
| :--- | :---: | :--- |
| `products` | 是 | 商品資訊，格式為 `款式ID:數量`，多項商品以 `,` 或 `;` 分隔 |
| `type=product` | 否 | 改用「商品 ID」取代「款式 ID」，系統自動取第一個款式 |
| `coupon` | 否 | 自動套用指定折扣碼 |
