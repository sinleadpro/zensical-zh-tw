...
完整的「配送方式 × 配送進度 × 退貨狀態」對應矩陣，以及每個欄位的詳細說明，請見 [訂單發送條件對照表](../references/c2c-return-conditions.md)。
...
2. **前往門市取貨**：商家需在包裹到店後 **7 天內** 攜帶 **與「姓名」欄位一致的身分證件正本** 前往門市領取，逾期未取時，包裹會自動改以 **宅配到付** 方式退回「[公司物流地址](../../website-management/設定網站基本資訊.md){ data-preview }」(由商家負擔宅配運費)。
3. **遇到門市關轉時**:
...
- :lucide-settings:{ .lg }  
  [__公司物流地址設定__](../../website-management/設定網站基本資訊.md)  
...
??? quote "為什麼訂單列表的批次動作裡找不到「發送 7-11 C2C 退貨寄件代碼」？"
    ...
    - 勾選的訂單是否符合所有發送條件 — 詳見 [訂單發送條件對照表](../references/c2c-return-conditions.md){ data-preview }
...
- [訂單發送條件對照表](../references/c2c-return-conditions.md)
