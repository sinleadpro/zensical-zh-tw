...
- **公司物流地址**：若尚未填寫，請於視窗下方「公司物流地址」區塊補上；此處的地址會作為新竹物流託運單的寄件人地址。點擊 「更改」可編輯該次出貨地址。
...
| 檔案 | 用途 |
| :-- | :-- |
| 託運單-寄件資料 | 黏貼於紙箱表面(請務必使用新竹物流專用紙列印) |
...
2. **司機收件**：司機到場時請出示 **新竹物流託運總表**(下載檔案中的第 2 份 PDF)供司機點收。
3. **狀態更新**：司機收件後，訂單詳情頁內會自動更新為 **「[已出貨(配送中)][shipping-status-text-type]{ data-preview }」**。

!!! tip "若多日無人收貨"
    可電聯新竹物流所屬營業所，提供寄件客代 **0577349** 並說明「已有印好的託運單」，請站所安排取件。
...
新竹物流有 **尺寸 / 重量上限** 與 **低溫不受理品項** 限制，出貨前請逐一檢查您的包裹是否符合，完整內容請參考：

- [尺寸與重量上限](references/新竹物流配送尺寸與運費對照表.md#hct-rate-card-limits){ data-preview }
...
若一筆訂單需 **拆成多箱寄送**(例如商品數量多、單一紙箱裝不下)，請改用 [加印託運單][hct-setup-reprint]{ data-preview } 功能，在「新竹物流託運單」頁面輸入同一訂單編號，產生新的單號與託運單。
...
- :lucide-printer:{ .lg }  
  [__設定新竹物流託運單__][hct-setup]{ data-preview }  
...
??? quote "「更多操作」裡找不到新竹物流下載選項?"
    #### 「更多操作」裡找不到新竹物流下載選項？ [](){ #faq-hct-shipping-action-missing } { .hidden-header }
    請依以下順序檢查:
...
??? quote "一筆訂單需要分多箱寄送，怎麼辦?"
    #### 一筆訂單需要分多箱寄送，怎麼辦？ [](){ #faq-hct-shipping-multi-box } { .hidden-header }
    請使用 **加印託運單** 功能:至「金物流」>「新竹物流託運單」>「加印託運單」，輸入同一訂單編號即可產生新的單號與託運單。每張加印託運單會 **個別扣費**。詳見 [加印託運單操作說明][hct-setup-reprint]{ data-preview }。

---

## 參考資料 { #reference-hct-shipping }

- [設定新竹物流託運單][hct-setup]{ data-preview }
- [新竹物流配送尺寸與運費對照表][hct-rate-card]{ data-preview }
- [訂單部分出貨](./partial-shipment.md){ data-preview }

[hct-setup]: ../shipping-notes/hct.md#intro-hct-setup
[hct-setup-reprint]: ../shipping-notes/hct.md#operate-hct-setup-reprint
[hct-rate-card]: ../shipping-notes/references/hct-rate-card.md#hct-rate-card
