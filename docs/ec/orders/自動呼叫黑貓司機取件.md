...
| 寄件人資訊 | 必須先至 **[黑貓寄取件設定頁][ezcat-shipping-note-sender-setup]{ data-preview }**(金物流 > 黑貓託運單 > 黑貓設定)完整填寫寄件人姓名、地址與聯絡電話，系統才會接受呼叫請求 |
...
6. **回答取件細節**：選「是」後會展開以下三個欄位：
    * **是否需在取件前事先電話聯絡**：選「是」時，司機抵達前會撥打「[黑貓寄取件設定頁][ezcat-shipping-note-sender-setup]{ data-preview }」中填寫的聯絡電話與您確認。
...
??? quote "已勾選「是」並送出，但司機沒有來?"
    ...
    2. [黑貓寄取件設定頁][ezcat-shipping-note-sender-setup]{ data-preview } 中的姓名、地址、電話是否完整填寫。
