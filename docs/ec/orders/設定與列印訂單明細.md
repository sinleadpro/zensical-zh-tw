...
- 新版選單版本：**金物流** > **結帳頁 & 物流設定** > 找到 **「列印訂單明細相關文件設定」**
- 舊版選單版本：**線上購物設定** > **購物車與金流** > 找到 **「列印訂單明細相關文件設定」**

> 進入後，區塊副標會顯示：設定訂單列表的列印明細顯示項目
...
> :lucide-asterisk: 已儲存的勾選選項不會受到影響。
...
!!! tip "提示"
    若您執行「確認出貨」並下載物流託運單，系統產生的 [壓縮檔][orders-print-fulfillment-zip]{ data-preview } 內亦會自動包含該筆訂單的明細檔案。
