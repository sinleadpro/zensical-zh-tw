...
2. **勾選本次出貨商品：** 在「出貨」區塊中，勾選本次要優先寄送的商品。如需一次選取全部商品，可點擊「選擇全部」；若需重新選擇，則可點擊「取消全部」。
    ...
    !!! warning "宅配貨到付款 (COD) 限制"
         若訂單為「貨到付款」，系統將不會顯示部分出貨選項。如需分箱寄送，請改用 「[加印託運單](../../../payments-and-logistics/reprint-waybills.md)」 功能，以確保代收款金額能正確分配至多組單號。
...
- **缺貨處理**：若門市庫存不足，可勾選實際有現貨的品項先行出貨。詳細流程請參閱 [缺貨訂單部分配送或取消指引](../../orders/out-of-stock-partial-shipping-or-cancellation.md){ data-preview }。
...
若您 **不是因為缺貨** 而要分批出，而是「所有商品都要一次寄出，但需要拆成多個包裹」，建議改用 「[加印託運單](../../payments-and-logistics/reprint-waybills.md){ title="補印與加印託運單" }」 功能:
