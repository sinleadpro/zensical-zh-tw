...
last_modified: 2026-06-30 10:45
lang: zh-TW
type: tutorial
status: ""
version: ""
...
![訂單詳情頁-部分出貨](../../../assets/images/EC-訂單-訂單詳情頁-部分出貨-hero.png){ title="訂單詳情頁-部分出貨" .hero-page }
...
??? info "不支援部分出貨的情境"
    ...
    * **已退貨的門市自取訂單**
    * **Amazon FBA 串倉訂單**
    * **完全來自串倉倉庫的訂單** ，且商家尚未加購「[部分串倉](../../../wms/enable-partial-warehouse-integration-and-order-splitting.md){ title="啟用部分串倉與拆單" }」功能
...
2. **勾選本次出貨商品：** 在「出貨」區塊中，勾選本次要優先寄送的商品。如需一次選取全部商品，可點擊「選擇全部」；若需重新選擇，則可點擊「取消全部」。
    ...
    ![訂單詳情頁-部分出貨](../../../assets/images/EC-訂單-訂單詳情頁-部分出貨.png){ title="訂單詳情頁-部分出貨" }

    !!! warning "宅配貨到付款 (COD) 限制"
          若訂單為「貨到付款」，系統將不會顯示部分出貨選項。如需分箱寄送，請改用「加印託運單」功能，以確保代收款金額能正確分配至多組單號。

3. **選擇出貨方式：** 在下方「選擇出貨方式」的下拉選單中，選擇本次要使用的物流(例如「黑貓託運單」)。

    ![訂單詳情頁-出貨方式](../../../assets/images/EC-訂單-訂單詳情頁-出貨方式.png){ title="訂單詳情頁-出貨方式" }

    !!! note "下拉選單僅會顯示 已開通的物流方式 及 該訂單可使用的物流選項。"

4. **選擇運費規格：** 在「請選擇費用」下拉選單中，挑選對應的託運單規格(如重量、尺寸區間)。

    ![訂單詳情頁-選擇費用](../../../assets/images/EC-訂單-訂單詳情頁-選擇費用.png){ title="訂單詳情頁-選擇費用" }
...
7. **檢視出貨紀錄**：
    - **商品出貨紀錄**：已出貨商品下方會顯示出貨時間與「快遞單號」。
      ...
      ![訂單詳情頁-部分已出貨](../../../assets/images/EC-訂單-訂單詳情頁-部分已出貨.png){ title="訂單詳情頁-部分已出貨" }

    - **訂單配送狀態**：訂單上方的配送狀態將更新為 「部分出貨」。

      ![訂單列表-配送狀態-部分出貨](../../../assets/images/EC-訂單-所有訂單-配送狀態-部分出貨.png){ title="訂單列表-配送狀態-部分出貨" }
...
當剩餘缺貨商品補齊後，只需要回到同一筆訂單，[重複上述步驟](#fuilfillment-partial-operate) 即可完成後續出貨：
...
!!! info "重要規範"
    ...
    | **自動化限制** | 部分出貨訂單 **不會自動結案**，需於全數出貨後[手動結案](../order-settings/manual-order-close.md){ title="如何手動結案訂單" }。 |
...
當您使用合作的第三方物流或自行配送時（詳細操作請參考[如何使用自訂物流出貨](custom-logistic-shipping.md){ title="如何使用自訂物流出貨" }）：
...
- **缺貨處理**：若門市庫存不足，可勾選實際有現貨的品項先行出貨。詳細流程請參閱 [缺貨訂單部分配送或取消指引](../../orders/out-of-stock-partial-shipping-or-cancellation.md){ title="缺貨訂單部分配送或取消流程" }。
...
當訂單同時包含「自行出貨商品」與「倉庫商品」時，在已加購「[部分串倉](../../../wms/enable-partial-warehouse-integration-and-order-splitting.md){ title="啟用部分串倉與拆單" }」功能的情況下，**自行出貨** 的部分可依照上述操作步驟先行出貨。倉庫出貨的部分則由倉庫端進行，不在本流程內。
