...
??? info "不支援部分出貨的情境"
    ...
    * **已退貨的門市自取訂單**
    * **Amazon FBA 串倉訂單**
    * **完全來自串倉倉庫的訂單** ，且商家尚未加購「[部分串倉](../../../wms/啟用部分串倉與拆單.md){ title="啟用部分串倉與拆單" }」功能
...
2. **勾選本次出貨商品：** 在「出貨」區塊中，勾選本次要優先寄送的商品。如需一次選取全部商品，可點擊「選擇全部」；若需重新選擇，則可點擊「取消全部」。
    ...
    !!! warning "宅配貨到付款 (COD) 限制"
        若訂單為「貨到付款」，系統將不會顯示部分出貨選項。如需分箱寄送，請改用 「[加印託運單](../../../payments-and-logistics/補印與加印託運單.md)」 功能，以確保代收款金額能正確分配至多組單號。
...
當訂單同時包含「自行出貨商品」與「倉庫商品」時，在已加購「[部分串倉](../../../wms/啟用部分串倉與拆單.md){ title="啟用部分串倉與拆單" }」功能的情況下，**自行出貨** 的部分可依照上述操作步驟先行出貨。倉庫出貨的部分則由倉庫端進行，不在本流程內。

## 何時建議改用「加印託運單」

若您 **不是因為缺貨** 而要分批出，而是「所有商品都要一次寄出，但需要拆成多個包裹」，建議改用 「[加印託運單](../../payments-and-logistics/補印與加印託運單.md){ title="補印與加印託運單" }」 功能:
