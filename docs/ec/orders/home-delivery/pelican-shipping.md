...
- [x] **宅配通寄件人資料**：至「金物流」>「宅配通託運單」頁面的「宅配通設定」區塊填寫，詳見 [宅配通託運單管理頁][configure-pelican-shipping-note-sender]{ data-preview } 。
...
* 退費後若仍使用該張託運單出貨，系統會 **再次記錄運費** (一般版扣 Cyber 幣；PLUS版 / 企業版列入對帳單)。
