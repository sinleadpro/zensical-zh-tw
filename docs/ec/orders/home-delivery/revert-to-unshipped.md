...
![訂單調整為未出貨](../../../assets/images/EC-訂單-所有訂單-更多操作-未出貨-hero.png){ title="訂單調整為未出貨" .hero-page }
...
2.  **勾選訂單**：在訂單列表中，找到配送狀態（[快速篩選](../basics/search-filter-orders.md#orders-filter){ title="如何搜尋與篩選訂單" }）為 **「準備出貨」** 的訂單並勾選。
3.  **選擇操作**：點擊列表右上方的 **「更多操作」** 或 **「選擇操作」** 下拉選單。
4.  **執行變更**：在選單中點選 **「未出貨」** 選項。
5.  **完成調整**：點選後，該筆訂單的配送狀態即會變更為「未出貨」，此時消費者即可在官網前台看到取消訂單的按鈕（若商家有開啟 [顧客取消訂單功能](../basics/cancel-order.md#orders-cancel-customer){ title="如何取消訂單" }  ）。

=== "新版訂單列表"

    ![訂單狀態改為未出貨](../../../assets/images/EC-訂單-所有訂單-更多操作-未出貨.png){ title="訂單狀態改為未出貨" }

=== "舊版訂單列表"


    ![訂單狀態改為未出貨-舊版訂單列表](../../../assets/images/EC-訂單-所有訂單-更多操作-未出貨-舊版訂單列表.png){ title="訂單狀態改為未出貨-舊版訂單列表" }
...
- :lucide-package:{ .lg }  
  [__訂單出貨流程__](../basics/order-fulfillment-flow.md){ title="訂單出貨流程" }  
...
??? quote "將訂單調回「未出貨」後，消費者可以取消訂單嗎？"
    ...
    可以。當配送狀態恢復為「未出貨」後，消費者即可在官網前台看到取消訂單的按鈕並執行取消操作（前提是商家已開啟 [顧客取消訂單功能](../basics/cancel-order.md#orders-cancel-customer){ title="如何取消訂單" }）。
