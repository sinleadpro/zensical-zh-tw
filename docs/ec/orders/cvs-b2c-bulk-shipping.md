...
permalink: https://help.cyberbiz.io/ec/orders/cvs-b2c-bulk-shipping
comments: false
search:
  exclude: false
...
5. **下載文件**：系統會自動產生託運單號並下載含託運資料的壓縮檔，訂單列表配送狀態變為「**已出貨**」，訂單詳情頁則顯示「**[已出貨（待物流收件）](shipping-status-tooltip.md#shipping-status-text-type){ data-preview }**」。
...
3. **物流中心簽收**：物流中心收件後，系統訂單狀態會由「待物流收件」自動變更為「**[已出貨（配送中）](shipping-status-tooltip.md#shipping-status-text-type){ data-preview }**」。
...
??? quote "如何確認物流中心已收到貨品？"
    ...
    物流中心簽收後，系統訂單狀態會由「待物流收件」自動變更為「**[已出貨（配送中）](shipping-status-tooltip.md#shipping-status-text-type){ data-preview }**」，您可在訂單列表或訂單詳情頁確認最新狀態。
