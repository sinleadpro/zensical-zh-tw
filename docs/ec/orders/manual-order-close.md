...
permalink: https://help.cyberbiz.io/ec/orders/manual-order-close
comments: false
search:
  exclude: false
icon: lucide/check
...
訂單完成結案後，可以透過訂單列表的篩選器建立「已結案」[訂單檢視群組][order-groups-tabs]{ data-preview }，以便確認已結案之訂單。
...
訂單詳情頁下方的 [訂單操作紀錄][order-history]{ data-preview } 會出現「訂單被關閉」的訊息，並顯示操作該結案動作的管理員名稱。
