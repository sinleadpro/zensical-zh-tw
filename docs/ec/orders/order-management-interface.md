...
lang: zh-TW
permalink: https://help.cyberbiz.io/ec/orders/order-management-interface
type: tutorial
status:
version:
author: Jase
...
- :lucide-check:{ .lg .middle } [__結案訂單__](manual-order-close.md){ data-preview }
...
![訂單詳情頁操作按鈕](../../assets/images/EC-訂單詳情頁-核心按鈕.png)
...
- **貨款資訊：** 顯示付款方式與狀態（如：等待付款、已收到款項）。
	...
	- 若顧客尚未付款，商家可在此複製[ **「付款連結」** ](provide-payment-link.md){ data-preview }  提供給顧客重新支付。
...
- **出貨操作：**
	...
	- **物流提示文字：** 針對串接物流，出貨後會顯示「**待物流收件**」或「**配送中**」等細節資訊。詳情請參閱 [物流提示文字說明](shipping-status-tooltip.md){ data-preview }  
