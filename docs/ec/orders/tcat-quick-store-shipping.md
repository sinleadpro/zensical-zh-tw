...
permalink: https://help.cyberbiz.io/ec/orders/tcat-quick-store-shipping
comments: false
search:
  exclude: false
icon: lucide/truck
...
!!! info "其他黑貓服務"
    * 若顧客選擇宅配，請見 [使用黑貓宅配出貨](tcat-home-delivery.md)。
...
5. **（選用）設定自動呼叫司機取件**：若商店已開通「[呼叫黑貓](tcat-auto-call-driver.md){ data-preview }」加值功能，視窗中會出現「是否自動呼叫黑貓司機取件」選項，選擇 **是** 後會展開以下三個欄位：
