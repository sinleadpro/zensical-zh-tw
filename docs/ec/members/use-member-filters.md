...
套用後，下方列表將即時顯示符合條件的會員名單。您可以：

- **儲存篩選器**：將此組條件儲存為常用篩選器，方便日後快速調用。
- **批次操作**：勾選會員後進行 [發送優惠券](manage-member-profiles/#2-優惠券派發與管理)、[匯出報表](manage-member-profiles/#任務一查看會員行為與資產數據) 或 [修改標籤](manage-member-profiles.md#任務五行銷標籤與分眾管理) 等操作。
