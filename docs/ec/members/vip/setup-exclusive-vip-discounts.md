...
設定各項優惠時，若遇到 **與其他行銷活動的併用規則** 欄目，請依此處說明選擇併用邏輯。

![](../../../assets/images/EC-後台-會員-VIP設定-與其他行銷活動的併用規則01.png){ .screenshot }

!!! info "紅利點數啟用開關"
    若想贈送紅利點數，請先前往 **行銷活動 > 全館折扣-紅利 & 優惠券**，[開啟紅利點數功能](../../marketing/bonus-and-gifts/setup-bonus-points/#操作流程)。
...
2. **設定與其他行銷活動的併用規則**
    ...
    請先選擇 [**併用邏輯**](#vip-優惠併用規則)，接著從清單中勾選欲套用此規則的行銷活動。
...
- **提前發送** ：您可自訂 **提前 N 天發送生日禮**。
    
    發送時依會員當下等級判斷是否發送。若發送時間點早於會員升等日期，該會員將無法收到升等後等級的生日禮。您可依商店的 VIP 政策，手動補送生日禮給會員。

    !!! example "情境舉例"
        顧客Ａ的生日在 5 月，並於 4/27 升等到 VIP1。
        系統設定每月提前 5 天發送該月生日禮，則 VIP1 5 月生日禮的發送時間點為 4/26。
        系統已於 4/26 發送 5 月生日禮，當時顧客 A 尚未升等，因此無法收到 VIP1 對應的生日禮。

- **排查時間**：當日凌晨 5 點，系統開始檢查符合條件的會員，並依序發送。
...
- **生日當月註冊**：若首階 VIP 門檻為 0 元，當月壽星註冊時會直接賦予該等級身分並發放生日禮，不受 **提前 N 天發送生日禮** 設定限制。
- **單次領取原則**：生日禮發送週期內（自提前發放日起至生日月底），每位會員限領取 **一次 VIP 生日禮**，且不因期間內升等而重複發送。

### 相關操作

- **疊加贈送**：若您於 **行銷活動 > 全館折扣-紅利&優惠券** 設定全館會員生日禮，消費者將 **同時獲得兩者**，請謹慎規劃生日禮發放政策。
- **刪除生日禮**：請至 **會員 > 所有會員**，點選個人頁面，[手動收回](../manage-member-profiles.md#任務四資產配置) 優惠券或紅利點數。
    ...
    === "優惠券"
        ...
        2. 選擇優惠券類型：
            ...
            - 贈品 **（企業版專用）**
                ...
                !!! info "VIP 贈品券使用規則"
                    1. 商品綁定規則：每張贈品券僅限綁定一個商品，且恕無法指定款式。
                    2. 多贈品設定方式：若欲一次贈送多個不同品項，請建立多筆贈品券。
                    3. 發送機制：系統將會全數發送所有綁定的贈品券，恕無法開放消費者於前台自行「擇一」領取。

                    → 了解 [贈品券完整規格](../../marketing/coupon/gift-coupon-spec/#使用須知)。
