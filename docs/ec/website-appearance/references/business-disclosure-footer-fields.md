---
title: 頁腳聯絡資訊欄位對照表
description: 官網頁腳「聯絡資訊」區塊可顯示的各項欄位與用途對照，包含統一編號、電話、地址等揭露資訊。
created: 2026-06-22 10:00
last_modified: 2026-06-22 10:00
lang: zh-TW
type: reference
status: ""
author: Jase
version: ""
reviewers: []
notes: []
ga_views: 0
feedback: 0
products:
  - EC
modules: []
sites:
  - TW
audiences:
  - admin
difficulty: ""
tnb: ""
plans: []
cyb_extensions: []
feature_badges: []
intents: []
features: []
prerequisites: []
related: []
tags:
  - 頁腳
  - 聯絡資訊
  - 統一編號
  - 營業人名稱
  - 揭露
  - 參考資料
acoiv: ""
apis: []
devices:
  - desktop
ui_components: []
paths: []
layouts: []
wp_url: []
permalink: https://help.cyberbiz.io/ec/website-appearance/references/business-disclosure-footer-fields/
comments: false
search:
  exclude: false
icon: lucide/list-checks
hide:
---

# 頁腳聯絡資訊欄位對照表

官網頁腳的「聯絡資訊」區塊可顯示下列欄位。每個資訊欄位都搭配一個「顯示」開關，需開啟開關該項才會出現在前台頁腳。

### 頁腳聯絡資訊欄位 { #footer-contact-fields }

| 欄位 | 說明 |
| :-- | :-- |
| 標題 | 聯絡資訊區塊在前台顯示的標題文字，預設為「聯絡資訊」 |
| 顯示聯絡電話 / 聯絡電話 | 顯示開關搭配電話內容，供消費者聯繫客服 |
| 顯示傳真號碼 / 傳真號碼 | 顯示開關搭配傳真號碼內容 |
| 顯示營業時間 / 營業時間 | 顯示開關搭配營業(客服)時間說明 |
| 顯示信箱 / 信箱 | 顯示開關搭配客服 Email |
| 顯示聯絡地址 / 地址 | 顯示開關搭配公司或門市地址 |
| 顯示統一編號 / 統一編號 | 顯示開關搭配統一編號，供揭露營業人統編使用 |

!!! note "註釋"
    * 每個資訊欄位都需先勾選對應的「顯示」開關，前台頁腳才會呈現該項內容。
    * 此處的「統一編號」為前台揭露用的獨立欄位，與後台「一般設定」的統一編號各自獨立，需分別填寫。
