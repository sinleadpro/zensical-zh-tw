...
last_modified: 2026-06-09 15:17
lang: "zh-TW"
type: "tutorial"
status: ""
version: ""
...
![自訂頁面管理](../assets/images/custom-pages-hero.png){ .hero-page }
...
4. **開始編輯：** 建立後即可點擊該頁面標題進入編輯，依您的頁面型態進行內容設計(見 [情境二][operate-custom-pages-edit]{ data-preview })。
...
=== "新版(拖拉式)頁面"
    ...
    1. **新增區塊：** 在編輯器中點擊 **「新增區塊」**，從清單選擇要加入的區塊(例如輪播素材、圖文介紹、折疊內容、自訂 HTML 等)。可用區塊請見 [自訂頁面可用區塊對照表][reference-custom-pages-blocks]{ data-preview } 。
    ...
    !!! note "註釋"
        拖拉式區塊的詳細操作與各區塊應用範例，請參考 [使用拖拉版型設計網站版面與首頁區塊](使用拖拉版型設計網站版面與首頁區塊.md) 。
...
!!! warning "注意"
    將頁面設為「公開」只是讓該網址可被開啟，並不會自動把連結放上官網選單。若要讓顧客在前台找到入口，還需到選單/導覽列加入連結(見 [後續操作][next-steps-custom-pages]{ data-preview })。
...
- :lucide-menu:{ .lg }  
  [__設定選單/導覽列__](menu-and-navigation.md)  
  頁面建立後不會自動出現在前台，需到「選單/導覽列設定」將頁面網址加入頁頭或頁腳選單。

- :lucide-layout-dashboard:{ .lg }  
  [__使用拖拉版型設計版面__](使用拖拉版型設計網站版面與首頁區塊.md)  
  了解新版拖拉式頁面各區塊的詳細設定與應用範例。

- :lucide-globe:{ .lg }  
  [__多國語言設定__](multi-language.md)  
...
??? quote "如何設定頁面的上架與下架時間？"
    ...
    排程功能僅適用於 **新版(拖拉式)** 頁面。請點擊該頁面進入佈景主題編輯器，於「檔期設定」設定上架時間與下架時間；未設定即視為無限期顯示。

    一般文字頁面沒有排程功能，狀態固定為「無限期」。
...
- [使用拖拉版型設計網站版面與首頁區塊](使用拖拉版型設計網站版面與首頁區塊.md)
- [選單/導覽列設定](menu-and-navigation.md)j
...
!!! info "瞭解 [各個區塊的詳細應用與範例](../theme-and-layout/使用拖拉版型設計網站版面與首頁區塊.md){ title="使用拖拉版型設計網站版面與首頁區塊" } 。"
