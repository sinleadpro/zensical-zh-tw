---
title: "部落格管理與文章發佈指南"
description: "透過部落格管理功能建立品牌文章，運用分類主題、文章標籤及 SEO 設定，建立內容行銷基礎，提升官網搜尋曝光率。"
created: "2026-03-19 11:27"
last_modified: 2026-06-30 11:01
lang: zh-TW
type: guide
status: update
author: Jase
reviewers: []
notes: []
ga_views: 0
feedback: 0
products:
  - EC
modules:
  - 網站外觀
  - 行銷活動
sites:
  - TW
audiences:
  - merchant
difficulty: beginner
tnb: trunk
plans:
  - 企業
  - 專業
  - 專業PLUS
  - 進階
  - 進階PLUS
  - 高手
  - 高手PLUS
cyb_extensions: []
intents:
  - 部落格
  - 內容行銷
  - 文章發佈
  - SEO 優化
features:
  - 部落格管理
  - 文章標籤
  - 拖拉版型
prerequisites: []
related:
  - "ec/website-appearance/site-settings/setup-og-image"
tags:
  - 部落格
  - 內容行銷
  - 文章管理
  - SEO
acoiv: operation
apis: []
devices:
  - desktop
  - mobile
ui_components: []
paths:
  - 網站外觀 > 部落格管理
layouts:
  - classic
  - draggable
wp_url:
  - https://www.cyberbiz.io/helpcenter/?p=631
  - https://www.cyberbiz.io/support/?p=1510
permalink: "https://help.cyberbiz.io/ec/website-appearance/pages-and-content/blog-management-publishing-guide/"
comments: false
search:
  exclude: false
icon: lucide/newspaper
hide: []
---

![部落格管理](../../../assets/images/ec-網站外觀-部落格管理.png){ title="部落格管理" .hero-page }

## 部落格管理功能說明

**部落格管理** 功能不僅能協助您撰寫品牌文章，更能透過分類主題、文章標籤以及完善的 SEO 設定，建立內容行銷的基礎，提升官網的搜尋曝光率。

## 新增部落格主題 <small>分類</small>

部落格主題可視為文章的大分類（例如：品牌新訊、產品開箱）。

*   **後台路徑**：網站外觀 > 部落格管理(文章/分類)。
*   **操作步驟**：點擊「新增部落格」按鈕，輸入主題名稱（如：品牌新訊）並點選「新增」即可。

![新增部落格主題](../../../assets/images/ec-網站外觀-部落格管理-新增部落格.png){ title="新增部落格主題" }

## 部落格排序

透過自訂排序功能，您可以彈性調整前台部落格主題的顯示順序，將熱門分類或季節性專題優先呈現給顧客。

1. **進入設定**： 登入 CYBERBIZ 管理後台，前往 「網站外觀」 > 「部落格排序」。
2. **手動排序**： 在列表中找到目標主題，按住左側的 拖曳圖示 :lucide-grip-vertical: 即可上下移動位置。
3. **即時儲存**： 調整完成後，系統將自動套用至前台顯示。

!!! tip "排版小技巧"
    建議將「最新消息」或「主打系列」置於首位，能有效提升訪客的點擊率與閱讀體驗。

![部落格排序](../../../assets/images/ec-網站外觀-部落格排序.png){ title="部落格排序" }

## 撰寫與管理文章

1.  **新增文章**：進入指定主題後，點擊右上方「新增文章」。

    ![新增文章](../../../assets/images/ec-網站外觀-部落格管理-新增文章.png){ title="新增文章" }

2.  **基本設定**：
    *   **內容編輯**：使用 [**文字編輯器**](../code-customization/text-editor.md){ title="使用文字編輯器編輯內容" }，支援上傳圖片、嵌入影片與表格等影音功能。

        !!! warning "請勿直接複製其他網頁或檔案內容貼上編輯器，建議使用「純文字貼上」按鈕（Ctrl+Shift+V）後再行上圖，以免帶入語法導致跑版。"

    *   **前台顯示欄位**：上傳代表圖片、輸入文章標題並設定標籤。

        ![文章圖片標題與標籤](../../../assets/images/ec-網站外觀-部落格管理-新增文章-圖片標題跟標籤.png){ title="文章圖片標題與標籤" }

    *   **字數限制**：每篇文章字元上限為 **65,535 字元**（以「原始碼」顯示之字數為準）。
3.  **列表操作**：建立完成的文章會出現在列表中，您可以點選圖示執行「**編輯、置頂、公開/不公開、刪除**」等動作，或點擊「作者」與「文章時間」進行修改。

        - :lucide-pencil:：點擊進入文章編輯頁面進行文章內容編輯設定。
        - :lucide-bookmark:：點擊將文章至頂 (前台顯示為第一篇文張)。
        - :lucide-eye:：公開(前台顯示)/不公開(前台不顯示)文章。
        - :lucide-x:：刪除文章。

        !!! tip "點擊列表中的文章標題可直接前往該文章的前台顯示頁面。"

        ![文章列表操作](../../../assets/images/ec-網站外觀-部落格管理-文章列表.png){ title="文章列表操作" }

## 文章標籤管理

[:lucide-tag:{ title="適用方案" }](../../../resources/conventions#適用方案){ title="圖示慣例" } | 企業

透過標籤（Tags）可以讓讀者更快速分類查找相關主題。

*   **後台路徑**：網站外觀 > 文章標籤管理。
*   **操作方式**：您可以新增文章標籤群組，並透過此功能 **個別或批次** 將文章加入特定標籤，或是將文章移出標籤群組。

    ![文章標籤管理](../../../assets/images/ec-網站外觀-文章標籤管理-加入移出標籤群組.png){ title="文章標籤管理" }

    !!! info "標籤群組詳細操作說明，可參考 [商品自訂分類群組](../../products/categories-and-tags/custom-collections.md){ title="設定商品自訂分類群組" }。"

- **前台呈現位置**：

    ![標籤前台呈現位置](../../../assets/images/ec-網站外觀-文章標籤-前台顯示位置.png){ title="標籤前台呈現位置" }

## 前台呈現與頁面設定

### 部落格頁 <small>摘要頁</small>

部落格頁會呈現多篇文章摘要。

![部落格頁前台](../../../assets/images/ec-網站外觀-部落格管理-部落格頁-前台.png){ title="部落格頁前台" }

您可以根據所使用的版型（拖拉版型或預設版型）來調整部落格的顯示樣式：

=== ":lucide-layout-dashboard: 拖拉版型"

    - **後台路徑** :「網站外觀」>「套版主題管理」>「網站設定」。 
    - **操作步驟**：選擇 **部落格頁** > 左側選單點擊 **部落格設定**。
    - **顯示設定**：
        - **每頁文章數項**：可自訂每頁要顯示的文章數，讓頁面長度更符合閱讀習慣。
        - **顯示精選文章**：勾選後，會在部落格頁底部顯示特定精選文章，便於曝光熱門或重要內容。
        - **精選文章群組**：選擇想呈現的精選文章來源群組，對應顯示內容。

    ![部落格頁設定-拖拉版型](../../../assets/images/ec-網站外觀-部落格管理-部落格頁設定-拖拉版型.png){ title="部落格頁設定-拖拉版型" }

=== ":lucide-layout-grid: 預設版型"

    - **後台路徑** :「網站外觀」>「套版主題管理」>「網站設定」。 
    - **操作步驟**：點擊展開 **部落格Blog** 設定選項。
    - **顯示設定**：
        - **顯示精選文章**：勾選後，會在部落格頁底部顯示特定精選文章，便於曝光熱門或重要內容。
        - **精選文章群組**：選擇想呈現的精選文章來源群組，對應顯示內容。

    ![部落格頁設定-預設版型](../../../assets/images/ec-網站外觀-部落格管理-部落格頁設定-一般版型.png){ title="部落格頁設定-預設版型" }

---

### 部落格文章頁 <small>單篇文章</small>

可設定版面內容資訊，並決定是否要 **顯示右側選單**，打造適合閱讀的版面。

![部落格文章頁前台](../../../assets/images/ec-網站外觀-部落格管理-部落格文章頁-前台.png){ title="部落格文章頁前台" }

您可以根據所使用的版型（拖拉版型或預設版型）來調整顯示樣式：

=== ":lucide-layout-dashboard: 拖拉版型"

    - **後台路徑** :「網站外觀」>「套版主題管理」>「網站設定」。 
    - **操作步驟**：選擇 **部落格文章頁** > 左側選單點擊 **部落格文章設定**。
    - **顯示設定**：
        - **顯示右側選單**：勾選後，會在文章右側顯示選單區塊，包括 **所有文章(部落格)主題**、**文章標籤區** 和 **最新文章**。
        - **文章標籤區標題**：可以設定文章標籤區塊的顯示標題文字。
        - **精選文章數量上限**：點擊展開選單設定精選文章顯示數量上限。

    ![文章頁設定-拖拉版型](../../../assets/images/ec-網站外觀-部落格管理-部落格文章頁設定-拖拉版型.png){ title="文章頁設定-拖拉版型" }

=== ":lucide-layout-grid: 預設版型"

    - **後台路徑** :「網站外觀」>「套版主題管理」>「網站設定」。 
    - **操作步驟**：點擊展開 **部落格Blog** 設定選項。
    - **顯示設定**：
        - **顯示右側選單**：勾選後，會在文章右側顯示選單區塊，包括 **所有文章(部落格)主題**、**文章標籤區** 和 **最新文章**。
        - **文章標籤區標題**：可以設定文章標籤區塊的顯示標題文字。
        - **精選文章數量上限**：點擊展開選單設定精選文章顯示數量上限。
        - **每頁文章數量**：點擊展開選單設定每頁顯示文章數量。

    ![文章頁設定-預設版型](../../../assets/images/ec-網站外觀-部落格管理-部落格文章頁設定-一般版型.png){ title="文章頁設定-預設版型" }

## SEO 與轉貼圖片設定

### Meta Tags

您可以針對「部落格主題」或「單篇文章」分別設定 **網頁標題、描述及關鍵字**，這有助於提高搜尋引擎曝光度。

!!! warning "關鍵字注意事項"
    - 請使用半形逗號 ( , ) 分隔多個關鍵字。正確範例： `女裝,夏季新品,洋裝`。
    - 關鍵字不會顯示於前台。

- **部落格主題**：登入後台，前往 **網站外觀** > **部落格管理** > 選擇 **部落格主題** > **部落格 Meta Tags 設定**。

    ![部落格Meta Tags設定](../../../assets/images/ec-網站外觀-部落格管理-部落格metatags設定.png){ title="部落格Meta Tags設定" }

- **單篇文章**：登入後台，前往 **網站外觀** > **部落格管理** > 選擇 **部落格主題** > **新增文章** 或點擊文章列表中的編輯按鈕 :lucide-pencil: 進入編輯頁面。

    ![文章Meta Tags設定](../../../assets/images/ec-網站外觀-部落格管理-文章編輯頁-文章標題簡述關鍵字.png){ title="文章Meta Tags設定" }

---

### 轉貼連結圖片 <small>OG Image</small>

在部落格主題或文章中設定的圖片，會同時影響前台主題封面以及分享連結至 Facebook、LINE 時出現的縮圖，讓分享內容更顯專業。

相關設定說明，請參考 [如何設定部落格文章轉貼連結縮圖](../site-settings/setup-og-image.md#部落格文章){ title="設定轉貼連結縮圖 OG Image" }。

## 常見問題

??? quote "每篇文章有字數限制嗎？"

    有的，每篇文章的字元上限為 **65,535 字元**（以「原始碼」顯示之字數為準）。

??? quote "文章標籤管理需要付費嗎？"

    文章標籤管理功能僅適用於 **企業** 方案，若您的方案不支援此功能，可以改用手動在文章內容中加入標籤關鍵字。

??? quote "如何讓文章出現在部落格頁的最上方？"

    在文章列表中點擊 :lucide-bookmark: 圖示，即可將文章置頂至部落格頁的最上方。
