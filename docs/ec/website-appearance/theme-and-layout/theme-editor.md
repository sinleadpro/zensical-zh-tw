---
title: 使用拖拉版型設計網站版面與首頁區塊
description: 使用拖拉版型編輯器建立與調整官網外觀，包含全站設定、頁面內容編輯、商品頁面配置等。
created:
last_modified: 2026-06-10 18:23
lang: zh-TW
permalink:
type: tutorial
status:
...
![拖拉版型編輯器](../assets/images/theme-editor-hero.png){ .hero-page }

## 功能介紹 { #intro-theme-editor }
...
整個編輯器分為三大區域:**左側區塊列表、中間即時預覽、右側區塊設定**,所有操作都圍繞這三個區域進行。
...
| 區塊設定 | 右側面板 | 點選區塊後展開,編輯該區塊的文字、圖片、連結、商品等內容 |
...
!!! plan "方案 / 開通條件"
    拖拉版型編輯器本身不限方案,只要您安裝並使用拖拉版型即可操作。若想確認店家是否已開通商品評論、門市據點列表等需加值的區塊,請聯繫您的 CYBERBIZ 業務窗口。
...
### 一、進入拖拉版型編輯器 { #operate-theme-editor-enter }

1. **開啟主題版型頁面:** 進入後台的 **「主題版型」** 頁面,這裡列出您商店已安裝的所有版型。
...
### 二、新增區塊 { #operate-theme-editor-add-section }
...
### 三、編輯區塊內容 { #operate-theme-editor-edit-settings }

1. **點選區塊:** 在左側列表點擊任一區塊,右側面板會展開該區塊的設定選項。
...
### 四、調整區塊順序 { #operate-theme-editor-reorder }

1. **找到拖曳把手:** 在左側區塊列表中,每個可調整的區塊都有一個拖曳圖示(移動把手)。
...
### 五、顯示或隱藏區塊 { #operate-theme-editor-visibility }

每個區塊都可以暫時隱藏,而不必刪除:

1. **點擊眼睛圖示:** 在左側列表中,點擊區塊上的眼睛圖示即可切換 **顯示** 或 **隱藏**。
2. **確認效果:** 被隱藏的區塊不會出現在前台,但您先前的設定內容會完整保留,日後重新顯示即可恢復。

---

### 六、移除區塊 { #operate-theme-editor-remove }

1. **進入區塊設定:** 點選欲刪除的區塊,展開右側設定面板。
2. **點擊移除:** 點擊 **「移除」**,該區塊即會從頁面中刪除。
...
### 七、切換編輯頁面與預覽 { #operate-theme-editor-preview }
...
### 八、復原、重做與儲存 { #operate-theme-editor-save }

1. **復原 / 重做:** 編輯過程中若要回到上一步,點上方的 **「上一動作」** 圖示;要還原則點 **「下一動作」**。
...
### 九、發布主題 { #operate-theme-editor-publish }
