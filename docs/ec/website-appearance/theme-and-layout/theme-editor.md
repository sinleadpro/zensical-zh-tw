...
created:
last_modified: 2026-06-13 18:31
lang: zh-TW
permalink:
type: tutorial
status:
...
商品標籤會依條件 **自動** 顯示於商品圖上，不需逐一手動標記。面板由上到下依序為 **特價、缺貨、定期定額、自定義標籤 1～5**。

!!! info "前置作業"
    **自定義標籤** 需綁定後台商品標籤，請先於「商品 > 商品標籤」建立標籤並套用至商品，才能在此綁定。
...
- **缺貨標籤：** 商品庫存為 0 且設為停止銷售時自動顯示。
- **定期定額標籤：** 商品被加入定期定額活動時自動顯示。
- **自定義標籤（每商品限 5 組）：** 各綁定一個後台「商品標籤」，套用該標籤的商品即自動顯示。全商店最多可建立 60 組。

---

**每個標籤的設定欄位**

點選任一標籤後，會依序出現以下欄位（部分為條件顯示）：

- **啟用核取方塊：** 勾選後該標籤才生效。
- **商品標籤（下拉）：** _僅自定義標籤_ —— 選擇要對應的後台商品標籤。
- **標籤類型：** 選擇呈現方式：
    - **文字：** 出現「標籤文字」欄，輸入要顯示的文字（按 ++enter++ 套用）。
    - **圖片：** 出現「圖片」上傳欄，並可選「圖片標籤大小」**大／小**（特價標籤預設大，其餘預設小）。
    - **折扣折數：** _僅特價標籤_ —— 系統依售價與定價自動換算折數呈現，無需手動輸入。
