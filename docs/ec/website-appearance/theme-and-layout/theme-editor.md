...
created:
last_modified: 2026-06-12 18:17
lang: zh-TW
permalink:
type: tutorial
status:
...
- **自定義標籤（最多 5 組）：** 各自綁定一個後台「商品標籤」，凡套用該標籤的商品就會自動顯示。
...
1. 於「全站共用設定 > 全站設定 > 商品標籤」找到要設定的標籤。
2. 勾選啟用，並（自定義標籤）綁定對應的後台商品標籤。
3. 選擇標籤類型，依類型填入文字或上傳圖片。
4. 於中間預覽確認後儲存。已發布版型即時生效；未發布版型需完成發布。
