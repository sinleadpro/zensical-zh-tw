...
此區設定會 **套用到全站所有商品列表與商品頁**（首頁商品區塊、商品群組頁、多層級分類、搜尋頁、商品詳情頁等），統一商品卡片與價格的呈現方式，不需逐一設定每件商品。
...
2. 依需求啟用各類標籤：
   ...
   - **自定義標籤（每商品限 5 組[^2]）：** 各綁定一個後台「商品標籤」，套用該標籤的商品即自動顯示。
...
- [導覽列與頁腳設定](../navigation/setup-menus-navigation.md)
- [互動遊戲設定指南](../../marketing/other-tools/interactive-games.md)
- [排程跑馬燈設定指南](configure-scheduled-carousels.md)
