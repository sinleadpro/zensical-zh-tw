...
- :lucide-shield-check:{ .lg }   
  [__網頁鎖右鍵功能__](../site-settings/設定網頁鎖右鍵保護圖文版權.md){ data-preview }       
  於 `<body>` 標籤植入 `onContextMenu` 語法，停用滑鼠右鍵選單以降低圖文內容被未經授權複製的風險。

- :lucide-search:{ .lg }     
  [__網站標題修改__](../site-settings/設定網站標題與 SEO.md){ data-preview }    
  透過修改 `{{ page_title }}` 邏輯，移除冗餘的預設歡迎詞，精準定義品牌在搜尋引擎結果頁（SERP）的呈現資訊。

- :lucide-timer:{ .lg }  
  [__首頁輪播圖轉場速度__](../theme-and-layout/調整首頁跑馬燈輪播圖的轉場停留時間.md){ data-preview }    
  於 `_main_slider.liquid` 修改 `delay` 數值，以毫秒為單位自訂首頁橫幅（Banner）的自動轉場速度。

- :lucide-mouse-pointer-2:{ .lg }  
  [__選單自動下拉__](../navigation/設定導覽選單滑鼠移入自動展開.md){ data-preview }     
  在 `main_nav.liquid` 嵌入 CSS 懸停（Hover）觸發邏輯，實現滑鼠移入時自動下拉展開次級選單，提升用戶導航效率。

- :lucide-filter-x:{ .lg }  
  [__搜尋排除特定關鍵字__](../../products/discoverability/設定搜尋結果中排除特定關鍵字商品.md){ data-preview }    
  利用 Liquid `without` 過濾器，從搜尋索引中排除特定標籤或關鍵字商品（如加價購、隱藏賣場），優化前台搜尋精準度。

- :lucide-globe:{ .lg }  
  [__前台語系與文字呈現__](../site-settings/設定前台語系與文字自定義.md){ data-preview }  
  自定義全站顯示文字與多國語系字典檔 (i18n)，調整按鈕名稱與系統預設語法。

- :lucide-message-square:{ .lg }  
  [__多元客服管道串接__](../customer-interaction/設置網站客服視窗.md){ data-preview }  
  在 `theme.liquid` </body> 前埋設 GetButton 代碼，或啟動 FBE 2.0，即刻建立官網即時通訊入口。

- :lucide-map-pin:{ .lg }  
  [__客服中心資訊調整__](../customer-interaction/設定與修改客服中心資訊.md){ data-preview }  
...
- :lucide-image-off:{ .lg }  
  [__鎖定圖片放大功能__](../theme-and-layout/關閉商品圖片放大預覽功能.md){ data-preview }    
...
- :lucide-type:{ .lg }  
  [__商品標語與簡述樣式__](../../products/create-and-manage/編輯商品簡述與商品標語.md){ data-preview }     
  透過 `theme_main.css` 定義專屬類別（Class），針對商品標語或規格描述進行排版客製，強化視覺層次感。
    
- :lucide-shuffle:{ .lg }   
  [__優先填寫收件人__](../checkout-and-shopping-flow/修改結帳流程為優先填寫收件人資訊.md){ data-preview }       
  於 `js/main.js` 啟用`exchangeShippingAndPurchaserLocation` 配置，使收件人欄位優先於購買人出現，符合跨境物流填寫習慣。

- :lucide-component:{ .lg }     
  [__訂單成立/付款完成頁客製化__](../../integrations/line/marketing/設定訂單成立頁與付款完成頁顯示 LINE 加入好友連結.md){ data-preview }    
