...
- :lucide-timer:{ .lg }  
  [__輪播圖轉場控制__](調整首頁跑馬燈（輪播圖）的轉場停留時間.md){ data-preview }    
  於 `_main_slider.liquid` 修改 `delay:` 數值（如 `5000` = 5秒），精確掌控首頁視覺節奏。

- :lucide-mouse-pointer-2:{ .lg }  
  [__選單自動展開__](設定導覽選單滑鼠移入自動展開.md){ data-preview }     
...
- :lucide-search:{ .lg }  
  [__自訂 SEO 網站標題__](設定網站標題與 SEO.md){ data-preview }  
  修改網站標題、描述與關鍵字，支援一般版型 `theme.liquid` 與拖拉版型設定。
...
- :lucide-image-off:{ .lg }  
  [__關閉圖片放大鏡__](關閉商品圖片放大預覽功能.md){ data-preview }    
  若商品圖解析度不足，可於 `product.liquid` 搜尋放大鏡代碼並加上 `//` 註解，避免模糊預覽。
    
- :lucide-play-circle:{ .lg }  
  [__一頁式影音自動播放__](../marketing/設定一頁式商店 YouTube 影片自動播放與起始秒數.md){ data-preview }     
