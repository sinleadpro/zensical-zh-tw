...
created:
last_modified: 2026-07-06T17:36:18+08:00
lang: zh-TW
...
=== "會員安全"
    ...
    | 會員密碼規則 | 規範會員密碼的長度與組成 | 開通功能（需聯繫客服開通） |
    | 會員個資部分隱碼 | 將顧客姓名、手機、地址等以隱碼遮蔽 | 開通功能（需聯繫客服開通） |
...
=== "管理員登入"
    ...
    | 管理員密碼規則 | 規範管理員密碼的長度與組成 | 開通功能（需聯繫客服開通） |
...
- [x] **方案／開通**：部分區塊需對應方案或開通功能才會出現，詳見下方折疊對照。
...
??? plan "各功能的方案與開通條件"
    ...
    | 二階段驗證 | 所有方案 |
    | 網站密碼 | 所有方案 |
    | 管理員／會員密碼規則 | 開通功能，需聯繫 CYBERBIZ 客服開通 |
    | 會員個資部分隱碼 | 開通功能，需聯繫 CYBERBIZ 客服開通 |
...
規範顧客註冊或設定會員密碼時的強度，操作方式與管理員密碼規則相同。此區塊需開通後才會顯示。
...
將顧客的姓名、手機、地址等個資以隱碼（如星號）遮蔽，可分別套用在不同位置。此功能需開通後才會顯示。
...
[^pdpa-scope]: 三個範圍各自對應不同的開通項目，只有已開通的範圍才會顯示對應勾選項。
...
??? quote "為什麼我在安全性設定看不到某些區塊？"
    ...
    - 管理員／會員密碼規則、會員個資部分隱碼為開通功能，需聯繫客服開通。
