...
| 區塊 | 設定內容 | 開通條件 |
| :-- | :-- | :-- |
| 關於您的網站 | 網站名、聯絡 Email | 所有方案 |
| 公司聯絡資訊 | 地址、電話、統一編號、公司全稱、帳務通知信箱 | 所有方案 |
| 公司物流地址 | 超商包裹退回的宅配到付退件地址 | 所有方案（使用電商倉儲服務時不顯示） |
| 調整標準和格式 | 訂單編號格式；幣別顯示格式 | 訂單編號格式為所有方案；幣別顯示格式需加值功能 |
| 後台顯示貨幣 | 顯示目前後台結算的幣別（唯讀，無法修改） | 所有方案 |
| 後台語言顯示 | 切換後台介面顯示的語言 | 所有方案 |
| 前台商店語言顯示 | 設定官網前台可切換的語言 | 加值功能，且需使用拖拉版型 |
| 前台商店幣值顯示 | 設定官網前台可切換的幣別 | 加值功能，且需使用拖拉版型 |
