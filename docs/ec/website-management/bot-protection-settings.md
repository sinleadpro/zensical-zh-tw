...
當官網已開通會員簡訊驗證（尤其含海外簡訊）時，可在發送驗證碼前加入機器人驗證，避免惡意程式短時間大量觸發簡訊，造成簡訊費用暴增。
{ .subtitle }
...
若你已啟用會員註冊簡訊驗證，且開通海外簡訊，網站可能被惡意程式反覆觸發「發送驗證碼」，在短時間內 **產生大量海外簡訊費用**。啟用此功能後，顧客點擊「發送驗證碼」前須先通過 Cloudflare Turnstile 機器人驗證，通過後系統才會寄出簡訊，降低此類攻擊造成的損失。
