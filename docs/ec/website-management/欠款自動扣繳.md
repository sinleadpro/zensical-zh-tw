---
title: 欠款自動扣繳設定
description: 當商家使用 CYBERBIZ PAYMENTS 金流服務時，若當期帳單因退貨退款等因素導致餘額為負值，系統將透過「欠款自動扣繳」機制從您綁定的信用卡中扣除差額，以確保站台功能正常運作。
...
# 欠款自動扣繳設定
當商家使用 CYBERBIZ PAYMENTS 金流服務時，若當期帳單因退貨退款等因素導致餘額為負值，系統將透過「欠款自動扣繳」機制從您綁定的信用卡中扣除差額，以確保站台功能正常運作。
{ .subtitle }
