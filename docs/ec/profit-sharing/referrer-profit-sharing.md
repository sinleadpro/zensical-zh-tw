...
5. 在 **基本設定** 區塊填寫以下資訊：
    ...
    - **顧客推薦人可以使用自己的推薦碼**：依需求開啟或關閉。
        ...
        - 若 **關閉** 此功能：系統會判定購物車內的會員是否與推薦人為同一人，以避免自領折扣；當顧客在未登入狀態下結帳時，系統因無法識別身分而無法發送推薦優惠。
        - 若 **開啟** 此功能：則系統無需驗證身分，無論顧客是否登入結帳，系統皆會發送推薦優惠。


        !!! note "登入門檻與優惠顯示邏輯"
            此開關將影響顧客是否需 **登入** 才能享有推薦優惠：
            
            - **開啟**：顧客 **無需登入** 即可在前台看到並享有推薦優惠。適用於希望降低轉單門檻、增加優惠曝光的活動。
            - **關閉**：系統會檢查登入狀態。顧客 **必須登入** 才能看到推薦優惠；若未登入，雖然結帳仍會計算分潤給推薦人，但顧客端無法享有推薦折扣。

            若您希望顧客結帳時皆能領取分潤優惠，建議將此項設為 **開啟**；或於 [顧客註冊設定](../website-management/customer-registration-flow-and-fields/#步驟-1選擇顧客註冊結帳模式) 中將站台設為 **必須註冊** 才能結帳。
...
2. **送優惠券 (會員)**：消費者達成門檻後，系統自動發送優惠券，供消費者下次使用。
    - **優惠種類**：可選「金額」或「百分比」折扣。
    - **消費門檻**：可指定金額門檻或不限金額。
    - **有效期限**：設定該優惠券的使用期限。
...
4. **送紅利 (顧客推薦人)**：消費者達成門檻後，推薦人可依訂單金額比例，獲得紅利點數。
    - **有效期限**：設定該紅利的使用期限。
