...
created: 2026-06-16
last_modified: 2026-06-27 21:21
lang: zh-TW
type: tutorial
status: ""
author: Jase
...
![購物車相關設定頁面](../../assets/images/ec-cart-settings-hero.png){ .hero-page }

## 購物車設定說明 { #intro-cart-settings }
...
| 設定區塊 | 用途 | 開通條件 |
| :-- | :-- | :-- |
| 購物車啟用設定 | 決定網站是否開放線上購買 | 所有方案 |
| 顧客購物車未結帳商品自動提醒 | 自動提醒購物車有遺留商品的顧客回來結帳 | 需加值功能(見下) |
| 結帳頁優惠券設定 | 是否在結帳頁顯示優惠券按鈕、自動套用全館優惠碼 | 所有方案 |
| 額外資訊 | 在結帳流程新增自訂詢問欄位 | 所有方案 |
| 定期定額購物車設定 | 設定定期定額活動的購物車預設商品數量 | 需加值功能(見下) |
...
!!! plan "方案 / 開通條件"
    * **顧客購物車未結帳商品自動提醒**：多數付費方案皆內建，僅「達人級」不支援。其中 **LINE OA 提醒** 管道需 PLUS版以上 或 企業版。
...
3. **設定提醒頻率：** 啟用任一管道後，下方會出現「提醒頻率」設定，最多可設定三次提醒，並各自填入「於第幾天送出」的天數(上限 14 天)。
4. **完成：** 點擊 **「儲存」** 套用設定。

各提醒管道的開通條件請見 [購物車未結帳提醒管道對照表][cart-reminder-channels]{ data-preview }。
...
* [購物車未結帳提醒管道對照表](references/cart-reminder-channels.md)
* [訂單相關設定](order-settings.md)
* [物流相關設定](logistics-settings.md)

[cart-reminder-channels]: references/cart-reminder-channels-reference.md
