...
features:
  ...
  - Uber Direct 優物流
prerequisites:
  - 若需使用進階指定配送功能，請確認已開通「進階指定配送」(PLUS版以上或企業版)
  - 若需使用退貨時間調整設定，請確認已開通對應功能
  - 若需使用 Uber Direct 優物流，請確認已開通該功能
related:
  - "[[購物車相關設定]]"
  - "[[訂單相關設定]]"
  - "[[設定與列印出貨明細]]"
...
!!! plan "方案 / 開通條件"
    * **配送指定日期設定**：開啟 / 關閉與「備貨天數」為基本功能；進階規則(配送日期必填、可選天數、每週固定日不出貨)需「進階指定配送」(PLUS版以上或企業版)；「設定特定日期不出貨」需「特定日期不出貨」功能。
    * **退貨時間調整設定**：需開通「退貨時間調整設定」功能(PLUS版以上或企業版)；其中「前台部分退貨申請」需另開通對應功能(企業版)。
    * **Uber Direct 優物流**：需開通「Uber Direct 優物流」功能。
...
4. **(進階)設定進階規則：** 若已開通進階指定配送，可額外設定 **配送日期必填**、 **可選天數範圍** 與 **每週固定日期不出貨**；若已開通特定日期不出貨，可點擊 **「前往設定頁面」** 在地圖式介面勾選特定不出貨日(如年節、連假)。
...
??? quote "Uber Direct 資料填錯了，可以改嗎？"
    [](){ #faq-logistics-settings-uber-edit }
    Uber Direct 商家資料一旦存檔即無法自行修改，部分情況僅商品類型可調整。若需更正其他資料，請聯繫 CYBERBIZ 業務窗口協助。
