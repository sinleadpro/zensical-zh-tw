...
調整配送的各項規範與顧客的送貨偏好:超商取貨姓名與離島、出貨明細列印、指定配送日期 / 時段、退貨可申請天數,以及 Uber Direct 優物流。
{ .subtitle }

## 功能介紹 { #intro-logistics-settings }
...
| 設定區塊 | 用途 | 開通條件 |
| :-- | :-- | :-- |
| 顧客禁用超商取貨付款設定 | 對多次未取貨的顧客自動禁用超商取貨付款 | 所有方案 |
| 超商取貨姓名規範 | 取件人姓名是否允許含英文數字 | 所有方案 |
| 超商啟用離島設定 | 是否開放 7-11 離島配送 | 所有方案 |
| 出貨明細列印相關設定 | 設定託運單出貨明細的顯示欄位 | 所有方案 |
| 配送指定日期設定 | 開放顧客指定配送日期(含進階規則) | 基本功能所有方案 / 進階需加值 |
| 配送指定時段設定 | 開放顧客指定配送時段 | 所有方案 |
| 退貨時間調整設定 | 設定前台可申請退貨的天數 | 需加值功能 |
| Uber Direct 優物流 | 串接快速到貨配送服務 | 需加值功能 |
...
各出貨方式的預設退貨天數與計算起點,請見 [前台可退貨天數對照表][return-eligible-days]{ data-preview }。
...
??? quote "退貨天數填 0 是什麼意思?"
    ...
    填入 0 代表 **不開放** 顧客在前台自行申請退貨。若希望使用系統預設天數,請將欄位 **留空**(CYBERBIZ 宅配 / 超取預設 10 天,其他出貨方式預設 14 天)。詳見 [前台可退貨天數對照表][return-eligible-days]{ data-preview }。
...
* [訂單相關設定](order-settings.md)

[return-eligible-days]: references/return-eligible-days-reference.md
