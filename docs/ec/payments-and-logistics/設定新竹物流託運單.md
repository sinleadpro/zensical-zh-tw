...
| 紙張規格 | 適用列印模式 |
| :-- | :-- |
| 10cm x 10cm 熱感應紙 | 訂單列表[下載][operate-hct-shipping-modal]{ data-preview }時選擇「熱感列印」、加印託運單時選擇對應紙張 |
| A4 六模託運單貼紙 | 訂單列表[下載][operate-hct-shipping-modal]{ data-preview }時選擇「一般列印(A4)」、加印託運單時選擇對應紙張 |
...
完成上一步後，請至「管理中心」>「一般設定」>「公司物流地址」 **再填一次相同的寄件地址**。這是因為訂單列表下載託運單時，系統會以「公司物流地址」為寄件人地址欄位。
...
2. **選擇貨品大小**：由下拉選單選擇對應的尺寸(常溫 60cm / 90cm / 120cm / 150cm / 160cm / 170cm / 180cm，或冷藏 / 冷凍 60cm / 90cm / 120cm)。下拉選單會即時顯示對應的 Cyber 幣費用。
...
頁面最下方提供「正物流」與「逆物流」兩張表，可查詢過往的託運單號、扣費紀錄、單號狀態：
...
- :lucide-truck:{ .lg }  
  [__使用新竹物流出貨__][hct-shipping]{ data-preview }  
  從訂單列表批次下載託運單並更新訂單貨態。

- :lucide-table:{ .lg }  
  [__新竹物流配送尺寸與運費對照表__][hct-rate-card]{ data-preview }  
  尺寸 / 溫層 / 不受理品項與 Cyber 幣費率。

- :lucide-map-pin:{ .lg }  
  [__一般設定__](#operate-hct-setup-company-address)  
  公司物流地址同步設定。
...
??? quote "為什麼地址驗證失敗?"
    #### 為什麼地址驗證失敗？ [](){ #faq-hct-setup-address-invalid } { .hidden-header }
    請依以下順序檢查：
...
??? quote "加印託運單需要扣費嗎?"
    #### 加印託運單需要扣費嗎？ [](){ #faq-hct-setup-reprint-fee } { .hidden-header }
    需要。**每張** 加印託運單都會 **個別扣費**，費用以您於下拉選單選擇的「貨品大小」對應的 Cyber 幣為準。一般版立即扣除;PLUS / 企業版列入對帳單。

??? quote "貨到付款訂單要怎麼加印託運單?"
    #### 貨到付款訂單要怎麼加印託運單？ [](){ #faq-hct-setup-cod-reprint } { .hidden-header }
    「加印託運單」 **不會** 列印貨到付款資訊。若您需要為貨到付款訂單分箱寄送，且每箱要各自代收款，請聯繫 CYBERBIZ 客服協助處理代收款分配。若不需分配代收款(只是分箱)，可使用「加印託運單」產生純配送單號搭配原本的代收款託運單。

??? quote "逆物流的收貨地址留白會怎樣?"
    #### 逆物流的收貨地址留白會怎樣？ [](){ #faq-hct-setup-reverse-address-blank } { .hidden-header }
    系統會自動填入 **原訂單的配送地址**(即顧客的收貨地址)作為新竹物流收件地點。若您希望由其他地點取件(例如顧客已搬家)，請手動填寫新的取件地址。

??? quote "已建立的單號可以撤銷嗎?"
    #### 已建立的單號可以撤銷嗎？ [](){ #faq-hct-setup-cancel-tracking } { .hidden-header }
    商家後台 **無法自行撤銷** 已建立的單號。若您沒有實際寄件，系統會於 **14 天後** 自動將單號改為「取消寄件」並回補 Cyber 幣(僅一般版)。如需提前撤銷請聯繫 CYBERBIZ 客服。

??? quote "Cyber 幣不足怎麼辦?"
    #### Cyber 幣不足怎麼辦？ [](){ #faq-hct-setup-insufficient-balance } { .hidden-header }
    請至「管理中心」>「儲值中心」儲值。一般版商家必須先儲值 Cyber 幣才能加印託運單或建立逆物流。若您是 PLUS / 企業版，則不需儲值，費用列入對帳單。

??? quote "單號使用紀錄的「對帳備註」是什麼?"
    #### 單號使用紀錄的「對帳備註」是什麼？ [](){ #faq-hct-setup-reconcile-note } { .hidden-header }
    對帳備註會顯示系統與新竹物流對帳的結果，例如「超過時間未寄件，回補 Cyber 幣」、實際尺寸與預估不符的補扣金額等。若欄位為空，代表此筆單號尚未進入對帳流程。

## 參考資料 { #reference-hct-setup }

- [使用新竹物流出貨][hct-shipping]{ data-preview }
- [新竹物流配送尺寸與運費對照表][hct-rate-card]{ data-preview }

[hct-shipping]: ../orders/ship-with-hct.md#intro-hct-shipping
[hct-rate-card]: references/hct-rate-card.md#hct-rate-card
