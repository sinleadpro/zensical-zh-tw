...
- **拆單與混單模式比較**
    - **拆單模式**：入倉商品與自出商品 **分開結帳**，產生多筆訂單，運費分別計算。相關設定請參考 [啟用部分串倉與拆單](enable-partial-warehouse-integration-and-order-splitting.md)。
...
!!! tip "最佳實踐：建立全域物流組合"
    若購物車組合不符合任何已設定的 **混合物流**，系統將退回 **拆單模式** 進行結帳。若要全站商品能混單結帳，建議設定一組 **囊括所有自訂物流** 的混合物流，以確保混單成功。
...
- **退貨方式**：依照 EC [訂單退貨流程](../ec/orders/order-return-process.md)。
- **統一窗口**：不論是倉庫商品或自出商品，混合訂單的退貨包裹預設皆寄回 **商家指定地址**。
- **倉庫商品回倉**：若商家收到倉庫商品後欲退回 WMS 倉庫，需手動於 WMS 後台[建立退貨單（逆物流）](return-orders.md#方式二手動建立退貨單)。
...
- :lucide-hand-helping:{ .lg }   
  [__開放會員於前台申請退貨__](../ec/orders/member-return-request-feature.md)     
  若開放消費者提出退貨申請，請同步開啟前台申請退貨功能。

- :lucide-file-spreadsheet:{ .lg }   
  [__串倉物流設定位置對照__](logistics-settings-reference.md)     
  了解不同串倉模式下，宅配物流的設定位置。
