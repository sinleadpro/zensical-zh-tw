...
- **異常修復**：若執行手動收貨後發現商品實際上未取件（如包裹遺失），須手動啟動 [退貨流程](../ec/orders/returns-refunds/訂單退貨流程/#步驟-1啟動退貨與安排逆物流){ title="訂單退貨流程" }，以利執行 [倉庫驗退作業]()、[訂單退款作業](../ec/orders/returns-refunds/訂單退款流程.md){ title="訂單退款流程" }。
