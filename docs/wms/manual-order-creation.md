...
last_modified: 2026-08-13 17:10
lang: zh-TW
type: guide
author: Ann
reviewers: []
...
2. **選擇通路**：選取訂單所屬的銷售通路。若尚未新增通路，請先 [建立通路](channels.md)。
3. **預約倉庫出貨日期**：
...
3. **規格確認**：
    ...
    - 選擇 **商品效期** 或 **批號**（若該商品有啟用效期/批號管理）。
        ...
        > 選擇 **效期** 時可 **指定配貨方式**，配貨選項可參考 [商品效期配貨方式](#商品效期配貨方式)。
...
建立訂單時，若商品有啟用效期管理，可在 **商品效期選擇配貨方式**，決定系統要從哪一批庫存出貨。

=== "最短效期"

    - **系統如何配貨**：從您設定的日期起，依效期由近到遠出貨；可跨多個批次湊滿數量。
    - **您設定的日期代表**：最早可接受的效期（含當日）。晚於或等於此日期的批次都可用。
    - **當庫存不足**：自動用其他符合條件的批次補足，訂單中同商品可能有不同效期。

=== "最短單一效期"

    - **系統如何配貨**：從您設定的日期起，找一個庫存夠的單一效期批次一次出完。
    - **您設定的日期代表**：最早可接受的效期（含當日）。
    - **當庫存不足**：改找下一個庫存足夠的效期；配貨完成後，即使原批次後來進貨，也不會再回頭改批。


=== "指定效期"

    - **系統如何配貨**：只使用您指定的效期出貨，不會改用其他日期。
    - **您設定的日期代表**：完全鎖定的效期（必須完全相符）。
    - **當庫存不足**：訂單維持「缺貨中」，保留已配到的數量；該效期進貨後再自動補齊。
